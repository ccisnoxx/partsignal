...
- 打开 Issue 必须保留 `PublishedArticle FOR UPDATE`，再由既有预检查拒绝已有 `OPEN` 或曾以 `RETIRED` 解决的问题。Issue INSERT owner 只把 `sqlstate=23505` 且 `diag.constraint_name` 精确等于 partial unique index `uq_published_content_issues_one_open` 的失败映射为既有 `409 PUBLISHED_CONTENT_ISSUE_CONFLICT`；命中后先 root rollback，不查询 winner、不解析错误文本。`RETIRED` 仍是预检查语义，直接写入产生的 `23514` 与其他 Issue unique/FK/CHECK/NOT NULL/trigger/constraint trigger、其他 constraint 和缺失 diagnostics 都保持 unknown。
- Catalog 必须证明该 partial unique index 的表、列和 `status = 'OPEN'` predicate，并证明它没有 `pg_constraint` row；真实重复 OPEN INSERT 必须证明 `23505 + exact diag.constraint_name`。合规双 Session 以有界 `pg_stat_activity/pg_blocking_pids` 证明 loser 等待同一 Article 的 `SELECT ... FOR UPDATE`，winner 提交后 loser 由 precheck 失败；test-only bypass 只允许在两个测试 Session 中移除 Article `FOR UPDATE` 并同步旁路 Issue precheck，同时保留实际 Article 读取及全部数据库约束，且必须证明 loser 等待在真实 Issue INSERT。两条路径都不得用 `sleep()` 猜竞态。
- Issue 竞争只能提交一个 OPEN Issue；known loser rollback 后原 Session 可复用。失败不得新增第二个 Issue 或 Repair Task，不得改变 Article 身份/核验绑定、PublicationWork status/revision、GEO link、追加式历史或 SUCCESS AuditLog；winner 正常产生的 `OPEN_ISSUE` 派生投影不属于 loser 副作用。
...
必需 PostgreSQL 证据包括 current-head catalog 中上述唯一约束、OPEN Issue partial unique index predicate 和 FK delete action、两个独立 Session 的真实锁等待/单赢家竞争、测试专用旁路触发的真实 `23505/diag.constraint_name`，以及已知冲突 rollback 后 Session 可复用。HTTP 测试必须冻结 `REPAIR_TASK_EXISTS`、`IDEMPOTENCY_CONFLICT`、`PUBLICATION_IDENTITY_CONFLICT` 与 `PUBLISHED_CONTENT_ISSUE_CONFLICT` 的 `ErrorEnvelope`、message、空 details 和 body/header request ID；unknown 500 只断言不泄漏 SQL、表名、约束、数据库 message 或堆栈，不把默认 500 固化成公共合同。成功与失败路径都要回归事件时间、追加式历史、删除事务、revision/state、AuditLog 和 GEO link 原子性。
...
- `GET /api/v1/platform-profiles/{id}` 对 CurrentUser 开放，在首次查询前建立 `REPEATABLE READ`，返回 `profile/account_summary/reference_summary/platform_type_options`；service 必须显式接收 `can_manage`。
...
- 合同生成与运行时测试覆盖新增字段、筛选参数、分页成对校验和删除 revision；PostgreSQL 集成测试覆盖三种 readiness、缺 Prompt 优先级、搜索筛选、分页、全集汇总、稳定选项、Detail ADMIN/ENGINEER 投影、404、repeatable-read、sparse/dense 固定查询次数及启停删除命令。
