...
export function AIChannelDetailPage() {
  ...
  return <div className="page-stack">
    {modalContext}
    <PageHeader eyebrow="MODEL GOVERNANCE" title={channel.data.name} description="渠道凭据和敏感 Header 永不回显；连接变更会重置模型测试状态。" breadcrumbs={[{ title: <Link to="/configuration/ai">AI 配置</Link> }, { title: channel.data.name }]} actions={<Space wrap><StatusTag status={channel.data.is_enabled ? 'ACTIVE' : 'RETIRED'} /><Button loading={toggleChannel.isPending} onClick={() => toggleChannel.mutate()}>{channel.data.is_enabled ? '停用渠道' : '启用渠道'}</Button><Popconfirm title="删除此 AI 渠道？" description="渠道、Header 与模型配置将被删除，此操作不可撤销。" okText="删除" cancelText="取消" okButtonProps={{ danger: true }} onConfirm={() => deleteChannel.mutate()}><Button danger icon={<DeleteOutlined />} loading={deleteChannel.isPending}>删除渠道</Button></Popconfirm></Space>} />
