...
export function ProductsPage() {
  ...
  return (
    <div className="page-stack">
      <header className="page-heading"><div><Typography.Text className="eyebrow">FACT FOUNDATION</Typography.Text><Typography.Title>产品事实</Typography.Title><Typography.Paragraph>先建立可审核、带证据的事实，再进入内容生成。</Typography.Paragraph></div>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>新增产品</Button></header>
      <Card>
        ...
        {products.error && <Alert type="error" showIcon message={errorMessage(products.error)} />}
        <Table<Product> rowKey="id" loading={products.isLoading} dataSource={products.data?.items} pagination={{ pageSize: 20 }} columns={[
          { title: '型号', dataIndex: 'part_number', render: (value, item) => <Link to={`/products/${item.id}`}><strong>{value}</strong></Link> },
