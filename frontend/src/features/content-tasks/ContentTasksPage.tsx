...
function TaskList() {
  ...
  return <div className="page-stack"><header className="page-heading"><div><Typography.Text className="eyebrow">CONTENT PIPELINE</Typography.Text><Typography.Title>内容任务</Typography.Title><Typography.Paragraph>任务锁定事实版本和平台规则，后续更新不会静默漂移。</Typography.Paragraph></div>{auth.hasRole('CONTENT_EDITOR') && <Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>创建任务</Button>}</header>
...
function TaskCreateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <Modal title="创建内容任务" open={open} onCancel={onClose} footer={null} width={760} destroyOnHidden>{create.error && <Alert className="form-alert" type="error" message={errorMessage(create.error)} />}<Form<Schema<'ContentTaskCreate'>> form={form} layout="vertical" onFinish={(body) => create.mutate(body)}><Space align="start" wrap>
    ...
    <Form.Item name="platform_profile_version_id" label="平台规则版本" rules={[{ required: true }]}><Select options={platforms.data?.items.map((item) => ({ value: item.active_version.id, label: `${item.name} V${item.active_version.version}` }))} /></Form.Item>
...
function TaskDetail({ taskId }: { taskId: string }) {
  ...
  const createJob = useMutation({ mutationFn: async () => unwrap(await api.POST('/api/v1/content-tasks/{content_task_id}/generation-jobs', { params: { path: { content_task_id: taskId }, header: { ...csrfHeader(), 'Idempotency-Key': newIdempotencyKey() } } })), onSuccess: async () => queryClient.invalidateQueries({ queryKey: ['generation-jobs', taskId] }) });
  ...
  return <div className="page-stack"><Button type="link" onClick={() => navigate('/tasks')}>← 返回任务列表</Button><header className="page-heading"><div><Typography.Text className="eyebrow">TASK / {taskId.slice(0, 8)}</Typography.Text><Typography.Title>{task.data?.content_angle ?? '内容任务'}</Typography.Title><Typography.Paragraph>{task.data?.target_audience} · {task.data?.desired_format}</Typography.Paragraph></div><Space><StatusTag status={task.data?.status ?? ''} />{auth.hasRole('CONTENT_EDITOR') && task.data?.status === 'OPEN' && <><Button onClick={() => setCommand('complete')}>完成任务</Button><Button danger onClick={() => setCommand('cancel')}>取消任务</Button><Button type="primary" icon={<ThunderboltOutlined />} loading={createJob.isPending} onClick={() => createJob.mutate()}>生成草稿</Button></>}</Space></header>
    {(task.error || versions.error || jobs.error || createJob.error || retryJob.error || taskCommand.error) && <Alert type="error" message={errorMessage(task.error ?? versions.error ?? jobs.error ?? createJob.error ?? retryJob.error ?? taskCommand.error)} />}
    <Card title="任务约束"><Descriptions column={{ xs: 1, md: 2 }} items={task.data ? [{ label: '事实版本', children: task.data.fact_version_id }, { label: '平台规则版本', children: task.data.platform_profile_version_id }, { label: '转化目标', children: task.data.conversion_goal }, { label: '官网 URL', children: <a href={task.data.canonical_url} target="_blank" rel="noreferrer">{task.data.canonical_url}</a> }] : []} /></Card>
    {latestJob && <Alert showIcon type={latestJob.status === 'FAILED' ? 'error' : latestJob.status === 'SUCCEEDED' ? 'success' : 'info'} message={<Space>最新生成作业 <StatusTag status={latestJob.status} /> 第 {latestJob.attempt_count} 次尝试</Space>} description={<Space wrap>{latestJob.error_summary ?? (latestJob.content_version_id ? <Link to={`/content/${latestJob.content_version_id}`}>打开生成的草稿</Link> : 'Worker 正在基于已批准事实生成内容。')}{auth.hasRole('CONTENT_EDITOR') && latestJob.status === 'FAILED' && <Button size="small" loading={retryJob.isPending} onClick={() => retryJob.mutate(latestJob.id)}>重试</Button>}</Space>} />}
