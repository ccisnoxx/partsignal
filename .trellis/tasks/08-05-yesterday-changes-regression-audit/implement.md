...
- [x] 首次上线复验确认目标知乎平台没有任何发布账号，服务端因此返回空的 `publication-ready-items`；不伪造就绪状态或代替用户创建业务数据。
- [x] 深链补充目标平台 ID；未就绪时展示明确阻断并链接到该平台的发布账号配置，就绪时仍打开原确认表单。
- [x] 新增三条最小回归测试；前端完整 Vitest 187 项、首次定向 30 项、最终精确 3 项、TypeScript 与相关 ESLint 均通过。
