...
- 2 路分片已随 `a2e50bf` 推送；该 push 未产生自动 run，手动入口创建唯一新 run `31008888955`。
- Shard 1 为 14 files、81 tests、410.01s，全部通过；Shard 2 为 13 files、109 tests、633.29s，全部通过且无超时。用户确认手动 CI 仅作低频备用，接受该耗时并停止继续增加 shard。
- 主 `verify` job 失败于既有 E2E 环境和视觉门禁：11 个 Firefox/WebKit 用例因 workflow 只安装 Chromium 而无法启动；另外两个 Chromium 用例分别暴露 AI 渠道表格列重叠和 Prompt 深色截图差异。它们与本次 Vitest 分片无关，未在本任务中擅自修复。
- 上述 E2E 问题留待独立任务处理，不阻断“push 只备份、CI 非日常门禁”的当前任务收尾。
