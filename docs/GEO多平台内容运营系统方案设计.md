...
2. **内容策划**：拥有历史目标问题和直接绑定具体平台的产品驱动内容任务。
...
├── content-planning    历史目标问题和直接平台内容任务
...
`audit_logs` 是业务审计唯一来源，并由 PostgreSQL 触发器保持追加后不可修改或删除。成功事件与业务写入同事务提交；当前对事实版本状态转换、内容提交与审核、发布登记、GEO 观测、平台、平台 Prompt、AI 渠道与模型、用户状态与管理员标识、用户导出等关键命令，在业务事务回滚后以独立事务记录 `FAILED` 或 `DENIED`。请求解析、身份认证、会话与 CSRF 失败不属于业务命令审计。历史平台规则审计允许只读展示，但系统不再产生新规则审计。
