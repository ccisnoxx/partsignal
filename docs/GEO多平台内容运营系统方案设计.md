...
- `ContentTask.current_content_version_id` 是内容单主线的权威指针。人工首稿、AI 结果、自然化和修订都只能产生新当前版本；旧版本只读，被放弃的当前草稿记为 `ABANDONED`，被退回版本必须创建修订后才能再审核。
...
模型不返回或自报 `used_fact_ids`、`used_evidence_ids`、`required_disclosure_ids`。Markdown 是内容唯一可编辑正文源；系统可以按目标平台临时渲染为 HTML 或纯文本，但不得保存另一份可独立编辑的正文。任务以 `current_content_version_id` 唯一指向当前内容主线；审核、待发布和发布候选都只认该版本。旧版本、被退回版本和 `ABANDONED` 草稿保持只读，需要修改时创建新版本并重新审核。
...
- 只有任务 `current_content_version_id` 指向的版本能推进审核和发布；放弃当前草稿后指针回到可继续的上一主线。
...
发布、GEO、批准内容和审核历史在保留期间仍禁止原地修改。永久删除已归档任务是唯一跨内容、发布和 GEO 的整体删除例外；它清理被删目标的旧审计后只保留一条空详情删除墓碑。产品删除不会代替管理员逐个清理事实版本，平台删除绝不级联任务，配置硬删除也不引入通用级联框架。
