...
本地或 CI 运行根 `make e2e` 时适用。V1 Playwright 使用真实 PostgreSQL、Redis、API、
Worker、对象存储和浏览器，每次运行必须拥有独立数据库与临时存储；V2 Foundation
Playwright 只验证 production build artifact，通过显式 fixture 隔离认证启动请求，不连接
或冒充真实业务后端。
...
- 根 `make e2e` 先运行 V1 隔离 E2E，成功后运行 V2 Foundation smoke；任一阶段失败时根 target 非零。
...
| V2 build/preview、fixture 或浏览器断言失败 | V2 smoke 与根 `make e2e` 非零退出，不影响已完成的 V1 精确清理 |
...
- Good：V1 独立数据库 E2E 完成并精确清理，随后 V2 对真实 production artifact 完成 Foundation smoke。
- Base：V1 产品缺陷或 V2 artifact 缺陷使根入口失败；V1 已创建资源仍完整清理并保留真实失败。
- Bad：V1 对共享开发库运行，或 V2 运行时加入 mock fallback、连接真实业务后端、过滤失败请求。
...
Correct: 先确认 5173 与 Redis broker 逻辑库由本次 V1 E2E 独占 → allowlist 数据库与 mktemp 精确清理 → V2 production artifact + 显式 fixture smoke → 任一失败使根入口失败
```
