...
`0033_task_owned_history_delete` 允许普通用户删除 `CANCELLED` 任务及其生成作业、内容审核记录和 `DRAFT | PENDING_REVIEW | CHANGES_REQUESTED` 内容版本。存在 `APPROVED | SUPERSEDED` 内容、任一发布记录或发布修复来源时返回 `CONTENT_TASK_IN_USE`；任务列表与详情使用同一服务端动作投影，删除服务仍在锁内重新校验。

管理员删除其他当前开发数据时，服务先锁定目标并统计直接引用，冲突统一返回结构化 `409`：产品检查事实版本、内容任务和 GEO 观测；事实版本检查内容任务和内容版本；具体平台检查内容任务和平台账号；平台账号检查发布记录；平台类型检查具体平台；停用用户由既有业务外键阻断。无引用事实版本可在任意状态删除，其从属事实审核记录在同一事务显式清理并保留安全审计摘要；产品删除不会自动删除事实版本。停用用户删除会清理会话并保留审计历史。除 `0033` 明确允许清理的任务自有未批准历史外，其他删除不级联或改写业务历史。
...
- [ ] 已取消任务可连同生成作业、审核记录和未批准内容版本删除；已批准/曾批准内容、发布记录或发布修复来源返回真实引用冲突且保持不变。
