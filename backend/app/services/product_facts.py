...
def validate_fact_graph(db: Session, payload: ProductFactsBody, require_complete: bool) -> None:
    ...
    _ensure_unique([item.client_key for item in payload.reference_parts], "参考型号")
    _ensure_unique([item.client_key for item in payload.parameters], "参数")
    _ensure_unique([item.client_key for item in payload.replacement_relations], "替代关系")
    _ensure_unique([item.client_key for item in payload.evidences], "证据")
    _ensure_unique([item.client_key for item in payload.claims], "事实表达")
