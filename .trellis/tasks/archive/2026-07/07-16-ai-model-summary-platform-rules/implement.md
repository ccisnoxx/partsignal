...
- [x] 前后端契约和应用必须同一版本发布；任一层验证失败不部署。
- [x] 部署后只用正式登录态重复第 1 节只读模型摘要证据，确认页面资源哈希对应新发布版本；不在正式环境执行平台规则或事实删除写烟测，除非用户另行明确授权。
- [x] `0015`、`0016` 不改写业务数据。应用回滚恢复上一版本镜像即可；不自动执行 Alembic downgrade，若显式降级则只恢复旧触发器。
- [x] 模型问题在发布前后均未复现，因此未进入资源或业务代码回滚分支；若未来取得失败证据，仍必须整版回滚，不能用前端过滤掩盖服务端错误。
