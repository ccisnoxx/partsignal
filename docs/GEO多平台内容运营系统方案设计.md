...
第一阶段不开发跨平台一键发布。普通用户在系统中完成产品资料维护、内容生成和人工审核后，通过发布工作跟踪外部人工发布、结果登记和首次核验；首次核验成功后形成只读发布成果，才交给 GEO 观测。后续页面问题独立登记并回流新的内容任务，不改写发布成果。

系统使用模块化单体架构，不采用微服务。产品事实、内容版本、发布工作、发布成果、发布后问题和 GEO 观测分别拥有独立生命周期，并通过明确 ID 关联。未来接入官方 API 或 CDP 自动发布时，只增加发布执行适配器，不改变内容审核、首次核验、只读成果和效果监测模型。
...
- 停用不联动既有账号，也不改写 Prompt、内容任务、发布工作、发布成果、GEO 观测或审计历史；服务端在新建普通/修复任务、平台账号和发布工作前锁定平台并明确拒绝停用状态。
...
- 普通任务创建要求客户端请求键；同键同三字段只返回首次任务，同键异载荷明确冲突，不同键允许相同业务输入。服务端以 PostgreSQL 事务锁和唯一约束作为最终幂等权威。
...
- 平台 Prompt 模板与全局自然化 Prompt 分别使用各自唯一端点。保存和删除使用 revision 乐观锁；共享模板保存或删除前展示全部受影响平台，删除会原子解绑这些平台。冲突保留本地草稿，切换标签、模板、路由或刷新前提示未保存修改。
...
- 一个具体平台可以维护多个不同发布账号；账号平台归属创建后不可编辑。业务标签与内部运营账号标识可编辑，账号可停用和重新启用，只有管理员能物理删除未被发布工作引用的账号。
...
- 同一具体平台下，以不可变内容哈希识别同一篇内容：同一内容版本只有一个发布工作，同平台同内容哈希最多一个未关闭工作。工作失败后在原工作修正和复核，不通过换账号创建第二条成功链；不同具体平台互不阻断。
- “历史目标问题”统一命名为“GEO 问题库”，入口与观测记录、分析洞察并列；实体和历史关联保持不变。

### 2.9 2026-07-30 任务自有历史与用户清理

- 已取消内容任务可连同生成作业、内容审核记录和 `DRAFT | PENDING_REVIEW | CHANGES_REQUESTED` 内容版本物理删除；存在 `APPROVED | SUPERSEDED` 内容、任一发布工作或非空发布后问题修复来源时服务端返回结构化冲突，不清理任何下游历史。
...
### 2.11 2026-08-03 发布管理流程重设计

- 发布管理明确拆成 `PublicationWork`、`PublishedArticle` 和 `PublishedContentIssue` 三个生命周期：工作跟踪人工操作，成果保存首次核验成功的只读事实，问题描述成功发布后的页面异常。
- 发布工作状态为 `PREPARING | PLATFORM_REVIEW | AWAITING_VERIFICATION | ACTION_REQUIRED | COMPLETED | CLOSED`。失败核验继续待处理并追加快照，直到复核成功或用户带原因显式关闭。
- 首次成功核验原子形成只读发布成果并完成来源内容任务；显式关闭原子取消来源任务。成功成果不因后续页面问题自动回退；管理员显式永久删除无 GEO 下游引用的完整成果聚合时，来源任务在原平台仍存在时恢复为 `OPEN`，原平台已删除时转为 `CANCELLED`。
- GEO 新观测只消费没有开放问题且未退役的 `PublishedArticle`。打开问题、创建修复任务和解决问题分别是独立动作，任何一步都不代替其他步骤。
- 发布业务对象禁止无上下文直接删除。管理员可在实时预览、固定确认和 revision 校验后永久删除无 GEO 下游引用的成果聚合；存在 GEO 观测/引用或优化来源时精确阻断，不级联或静默解绑独立历史。

### 2.12 2026-08-04 业务主操作与单主线

- 有业务推进含义的资源由服务端返回领域内 typed `workflow_stage` 和唯一 `primary_task`；前端只负责把 token 映射为中文主操作，不从状态、角色或分页数据重建业务资格。`available_actions` 继续表达可尝试的具体命令，写入入口仍必须重新校验。
- 产品事实工作区以 `expected_revision` 一次提交并直接创建 `PENDING_REVIEW` 不可变快照；事实版本不再有 `DRAFT` 或独立提交命令，单个产品同时最多一个待审核版本。
- `ContentTask.current_content_version_id` 是内容单主线的权威指针。人工首稿、AI 结果、自然化和修订都产生当前版本；当前、未审核的人工 `DRAFT` 可按 revision 原地保存，AI 草稿和提交审核后的版本只读。无直接引用的人工 `DRAFT | ABANDONED` 可彻底删除，AI 草稿仍只能放弃；被退回版本必须创建修订后才能再审核。
...
- **人工把关**：所有外发内容必须经过明确审核。
- **版本可追溯**：发布工作与发布成果必须指向具体内容版本和事实版本。
- **发布方式解耦**：人工发布和未来自动发布共用同一工作、核验和只读成果模型。
...
6. 每个发布工作必须绑定内容任务和当前已批准版本，并只选择一个属于任务平台的账号；成功前只允许切换到同任务、同平台的当前已批准版本。
...
14. AI 或人工来源的内容都必须先批准，才能创建发布工作。
15. 失败核验必须继续待处理，直到复核成功或显式关闭；失败不得伪装成成功、自动取消或通过删除抹除。
16. 同一具体平台可以维护多个账号，但内部运营账号标识去除两侧空白并忽略大小写后必须唯一，停用账号仍保留历史身份。
17. 首次成功核验形成的 `PublishedArticle` 只读且不可删除；后续页面问题由独立对象表达。
18. 每个内容任务只有一个当前内容版本；审核、待发布和发布候选都只认该权威指针。
19. 资源的 `primary_task` 只由服务端根据 PostgreSQL 当前事实投影；前端不得以状态或角色推导另一套主操作。
...
3. 用户以当前 `facts_revision` 和变更说明原子提交审核；系统直接冻结新的 `PENDING_REVIEW` 版本，不产生事实草稿版本。
4. 用户核对冻结的 Markdown 原文与分级，并显式执行批准或退回；系统不以审核人与创建人是否相同作为阻断条件。
5. 审核通过后，正文和分级成为不可变的已批准事实版本。
6. 后续修改必须创建新版本，不允许直接修改已批准版本。

### 7.2 内容生成流程

1. 用户创建内容任务，只选择产品、该产品的非空已批准事实版本和活动具体平台；同一次弹窗提交与失败重试复用一个请求键，服务端保证只创建一条任务。
...
5. 首稿、AI 结果、自然化和修订每次都把新版本设为任务当前版本；用户只编辑、审核和发布当前版本。被退回版本只读，必须创建修订；系统 AI 重试仅允许 v2 快照并创建新作业。

### 7.3 人工发布流程

1. 已批准、绑定有效已批准事实且来源任务为 `OPEN` 的内容进入“待开始”列表。
2. 用户在按需 Drawer 中读取不可变发布包，只选择锁定平台下的一个匹配账号和目标栏目 URL；系统建立 `PREPARING PublicationWork` 并固定内容版本、平台、账号和内容哈希。
3. 用户复制标题、Markdown 或纯文本，在目标平台人工发布；如平台存在处理或审核阶段，通过显式命令进入 `PLATFORM_REVIEW`。
4. 发布完成后登记实际标题、最终文章 URL、发布时间、非空操作说明和可选结果证据；服务端校验域名并原子写入结果、证据、工作事件和审计，工作进入 `AWAITING_VERIFICATION`。
5. 人工核对页面正文与发布工作当前批准版本一致后执行首次核验。失败时追加含当时内容版本的快照并进入 `ACTION_REQUIRED`；用户可修正登记结果，或切换到同任务、同平台的新当前批准版本后继续复核。成功时原子形成只读 `PublishedArticle` 并完成来源内容任务。
6. 无法继续的非终态工作必须选择原因、填写说明并显式关闭；工作进入 `CLOSED`，来源任务进入 `CANCELLED`。关闭不可恢复，不提供删除替代。
7. 成功后的页面下线或内容变化创建 `PublishedContentIssue`；它不回退来源任务或改写成果，修复任务与显式解决分别执行。

### 7.4 GEO 观测流程

1. 选择一个产品，在豆包、DeepSeek、元宝等站外网站人工搜索。
2. 选择真实问题主题，并登记人工搜索平台、实际搜索词、测试时间和真实结果截图。
3. 系统列出该产品当前合格的全部 `PublishedArticle`；存在开放问题或曾退役的成果不进入新观测。
...
5. **发布管理**：拥有平台账号、发布工作、首次核验、只读发布成果和发布后问题。
...
| `PublicationWork` | 发布工作 | 跟踪某个已批准内容的人工发布过程、结果登记和首次核验 |
| `PublishedArticle` | 发布成果 | 保存首次核验成功后不可变的公开文章身份和结果 |
| `PublishedContentIssue` | 发布后内容问题 | 保存成功发布后的页面不可用、内容变化及显式处理结果 |
...
erDiagram
    ...
    CONTENT_VERSION ||--o| PUBLICATION_WORK : published_by
    PUBLICATION_WORK ||--o| PUBLISHED_ARTICLE : completes_as
    PUBLISHED_ARTICLE ||--o{ PUBLISHED_CONTENT_ISSUE : reports
    PUBLISHED_ARTICLE ||--o{ GEO_OBSERVATION : may_influence
...
`Product` 保存唯一可编辑的 `facts_body_markdown`、`facts_classification` 与 `facts_revision`。工作区提交审核时原子冻结 `FactVersion`：

- `body_markdown` 原文与 `classification`。
- 产品引用、版本号、状态、创建人和时间。
- 追加式审核记录、审核人和批准时间。

`FactVersion` 只有 `PENDING_REVIEW | CHANGES_REQUESTED | APPROVED | RETIRED`，不存在草稿阶段。已批准或退回的快照不允许更新；后续修订回到事实工作区并再次原子提交。
...
任务详情按生命周期互斥展示入口：尚无草稿时显示系统 AI 与人工首稿；进行中且已有当前内容时只显示当前内容工作；完成或取消后只读展示历史，并单独提供“基于当前上下文新建任务”。续建沿用产品和仍有效的平台，事实版本默认选择该产品最新已批准版本，创建后不会自动触发 AI。没有记录时隐藏“AI 生成记录”，加载和失败状态仍保留可恢复反馈。

普通任务还保存内部可空 `idempotency_key`。新建 API 要求 8–128 字符请求键；同键只能描述同一组产品、事实版本和具体平台，不同请求键不触发业务去重。历史任务与发布修复任务没有客户端创建意图，保持空值且不伪造回填。
...
平台停用后继续保留上述配置能力，但不能用于新建内容任务、修复任务、平台账号或发布工作。各新建服务与启停命令共享“先锁平台、再检查状态”的事务顺序，避免并发停用被检查后写入穿透；既有账号和全部历史事实保持原状。
...
模型不返回或自报 `used_fact_ids`、`used_evidence_ids`、`required_disclosure_ids`。Markdown 是内容唯一可编辑正文源；系统可以按目标平台临时渲染为 HTML 或纯文本，但不得保存另一份可独立编辑的正文。任务以 `current_content_version_id` 唯一指向当前内容主线；审核、待发布和发布候选都只认该版本。当前、未审核的人工 `DRAFT` 可保存到原版本；旧版本、被退回版本、AI 草稿和已审核版本保持只读，需要修改时创建新版本并重新审核。

#### `PublicationWork`、`PublishedArticle` 与 `PublishedContentIssue`

`PublicationWork` 保存来源内容任务、当前批准内容版本、内容哈希、任务平台下的一个账号、目标栏目、结果字段、当前阶段、revision、关闭事实和创建人。成功前可切换到同任务、同平台的新当前批准版本，每次切换和状态变化都追加 `PublicationWorkEvent`；每次核验追加当时 `content_version_id`、实际标题、最终 URL、发布时间、结论、说明、操作者和时间快照。发布证据通过追加式 `publication_attachments` 关联，只接受已验证的 `OPERATION_SCREENSHOT`。

失败核验不会产生新工作或终止来源任务，而是把同一工作置为 `ACTION_REQUIRED`。用户修正实际标题、URL 或发布时间，或切换新批准版本后重新进入待核验；首次通过时创建与工作同 ID 的 `PublishedArticle`，它只读投影成功核验快照中的内容版本、平台、账号、实际标题、最终 URL 和发布时间。工作与来源任务同时进入 `COMPLETED`。

`PublishedContentIssue` 只描述发布成功后的页面不可用、公开内容变化或其他问题。一个成果同一时刻最多一个 `OPEN` 问题；问题可创建一个来源明确的修复 `ContentTask`，但仍保持 `OPEN`，直到用户以 `RESTORED` 或 `RETIRED` 显式解决。成果及其历史不因问题打开、修复或解决而改写。

发布业务对象均不提供物理删除。无法继续的非终态工作通过带原因和说明的 `CLOSED` 结束，并取消来源任务；开发环境数据清理由环境重置承担，不进入业务模型。
...
- 事实工作区提交直接创建 `PENDING_REVIEW`，不存在事实版本 `DRAFT`。
- `APPROVED` 后不可修改。
...
- AI 输出固定为 `DRAFT`。
- 当前、未审核的人工 `DRAFT` 可按 revision 原地保存；提交审核后关闭该可变窗口。
- `CHANGES_REQUESTED` 是不可变退回结论，必须基于它创建新 `DRAFT`，不得原样重提。
- 只有任务 `current_content_version_id` 指向的版本能推进审核和发布；AI 草稿放弃后指针回到最近批准主线，人工草稿彻底删除后只恢复直接父版本或置空。
- 只有任务绑定明确具体平台的 `APPROVED` 内容才能创建发布工作。
...
- `AWAITING_VERIFICATION` 表示已经登记公开结果，等待人工首次核验。
- `ACTION_REQUIRED` 表示最近一次核验失败，工作仍必须继续处理。
- `COMPLETED` 只由首次成功核验产生，并同时形成只读 `PublishedArticle`。
- `CLOSED` 只由显式关闭产生，必须保留原因、说明、操作者和时间。
- 发布账号必须属于平台级内容锁定的具体平台；前端只展示匹配账号，服务端和 PostgreSQL 共同拒绝错绑。
- 一条发布工作只能选择一个账号；同内容版本只允许一个工作，同平台同内容哈希最多一个未关闭工作。
- 首次成功核验前可用 `expected_revision` 切换到同任务、同平台的当前批准版本；终态工作拒绝切换。
- 发布后页面问题不回退成果或任务，以独立 `PublishedContentIssue` 处理。
...
```text
OPEN → CANCELLED
OPEN -- 首次发布核验成功 --> COMPLETED
```

`COMPLETED` 记录任务已完成首次核验闭环，通常不回退；管理员永久删除完整发布成果聚合是显式例外：原平台仍存在时任务恢复为 `OPEN`，原平台已删除时任务转为 `CANCELLED`。`archived_at` 与业务状态正交，归档只改变默认可见性，恢复不改变当前状态。非终态发布工作存在时不能直接取消任务；关闭发布工作会在同一事务把来源任务置为 `CANCELLED`。生成和发布进度通过作业、内容版本与发布工作推导，不维护重复状态。

未归档 `OPEN | CANCELLED` 任务在没有运行中生成作业、成功文章和 GEO 文章关系时，可由普通用户删除整个未成功发布聚合。`COMPLETED` 必须先归档，随后只允许管理员通过实时预览、revision 和固定确认文本永久删除内部聚合；前端只消费服务端动作，不自行推导资格。
...
审核是内容进入待发布状态前必须完成的流程动作，不要求由另一账号执行。创建人可以审核自己的内容，但批准必须显式确认并生成 `ReviewRecord`；系统不得因操作者相同而跳过审核记录。事实与内容的退回意见去除空白后必须非空，提交和批准意见可选。审核记录不可删除；退回结束当前版本，后续处理必须创建新修订并追加新审核记录，不覆盖上一次意见。
...
发布工作台由服务端摘要和三类真实资源组成：

- 发布工作：默认展示待开始候选和全部非终态工作，`ACTION_REQUIRED` 与 `AWAITING_VERIFICATION` 优先；已关闭历史通过筛选查看。
- 发布成果：只展示首次核验成功形成的 `PublishedArticle`，正文和核验历史不可原地修改；管理员可在没有 GEO 下游引用时永久删除完整成果聚合。
- 内容问题：默认展示 `OPEN PublishedContentIssue`，详情独立展示原成果、修复任务和显式解决动作。
- 摘要只返回 `ready_count`、`active_count`、`awaiting_verification_count`、`action_required_count` 和 `open_issue_count`。
- Tab、筛选、分页和 Drawer 选中对象以 URL 为唯一导航状态；表单内容保留在组件局部。
...
结果证据复用既有文件直传和完整性校验能力，只接受已验证的 `OPERATION_SCREENSHOT`。文件上传成功但结果登记失败时，文件记录仍可追溯，但不会伪装为已绑定发布工作。

Drawer 的锁定上下文直接来自对应资源详情投影。旧发布详情、关注事项和独立修复路由已删除；所有状态写操作留在工作台 Drawer 与确认对话框中。候选发布包读取失败时必须展示真实错误并阻止复制与开始工作。

如果用户在平台上修改了正文，而不只是标题或排版，应先回到系统创建新的内容版本并审核，再在原发布工作切换到该版本后复核。不创建第二条发布工作，也不用更改结果字段代替版本切换历史。
...
服务端以独立核验命令保存 `PASSED | FAILED`、`content_matches`、说明、当时 `content_version_id` 及结果快照。`FAILED` 必须说明原因，工作进入 `ACTION_REQUIRED` 并继续待处理；`PASSED` 必须满足 `content_matches=true`，原子形成指向该核验版本的发布成果并完成来源任务。自动页面抓取不在当前范围。

### 12.4 发布异常与修复任务

发布成果形成后出现页面不可用、公开内容变化或其他问题时，创建唯一 `OPEN PublishedContentIssue`。问题详情保留原成果、原任务、问题类型和说明，不回退已完成内容任务，也不通过日常发布命令改写成果；管理员永久删除成果聚合时问题随聚合删除，独立修复任务保留并解除来源问题关联。

创建修复任务与解决问题是两个独立操作。修复任务固定继承原产品和直接平台；普通用户必须从同产品当前非空 `APPROVED` 事实版本中重新选择。创建任务只写唯一 `source_published_content_issue_id`，问题仍为 `OPEN`；显式解决必须选择 `RESTORED` 或 `RETIRED` 并填写说明，不自动完成修复任务。

### 12.5 发布工作台摘要

摘要从 PostgreSQL 当前状态实时聚合，不保存统计表或由前端遍历分页推算：

- `ready_count`：当前可开始发布且尚无工作的已批准内容数。
- `active_count`：`PREPARING | PLATFORM_REVIEW` 工作数。
- `awaiting_verification_count`：`AWAITING_VERIFICATION` 工作数。
- `action_required_count`：`ACTION_REQUIRED` 工作数。
- `open_issue_count`：`OPEN PublishedContentIssue` 数。

发布成果数量与 GEO 指标通过各自列表和洞察契约读取。历史工作、事件、核验和问题保留完整，但不再用旧周期状态事件口径混合当前待办与成果。

### 12.6 开发阶段删除与归档

删除规则收缩为资源所有者能够直接解释的几条命令；服务端锁定目标并重新检查，前端只消费 `available_actions` 和结构化阻断，不复制状态判断。外部文章由人工登记，任何任务删除都不验证或删除外部页面。

| 对象 | 当前阻断 | 成功后的边界 |
|---|---|---|
| 产品 | 事实版本、内容任务、GEO 观测 | 删除产品和当前事实工作区，不删除关联对象 |
| 事实版本 | 内容任务、内容版本 | 任意状态均可删；同事务清理从属事实审核记录 |
| 人工未审核草稿 | 审核、子版本、生成来源/结果、发布工作/事件/核验引用 | 无引用的 `HUMAN DRAFT | ABANDONED` 可彻底删除；AI 草稿不可删，当前指针恢复直接父版本或置空 |
| 未归档内容任务 | 运行中生成作业，或成功文章/GEO 文章关系 | `OPEN | CANCELLED` 可删除完整未成功发布聚合；`COMPLETED` 必须先归档 |
| 已归档内容任务 | 非管理员、revision/固定确认文本不匹配、运行中作业 | 管理员永久删除整个内部任务、内容、发布及独占 GEO 聚合；共享 GEO/文件保留 |
| 具体平台 | 平台仍启用、`OPEN` 任务、非终态发布工作 | 必须先停用；账号随平台清理，任务不级联，终态历史改读平台/账号快照 |
| 平台账号 | 非终态发布工作 | 终态发布历史改读账号快照 |
| 平台 Prompt | revision 冲突 | 自动解绑全部当前平台并递增平台 revision；历史生成快照不变 |
| 平台类型 | 具体平台 | 不自动删除具体平台 |
| 停用用户 | 业务历史外键 | 会话清理；保留审计的操作者引用受约束置空 |

`ContentTask.archived_at` 只控制默认可见性，不改变 `OPEN | CANCELLED | COMPLETED` 业务状态。完成任务可归档和恢复；默认列表排除归档任务。永久删除前返回实时分项数量和去重外部 URL，并要求管理员输入 `永久删除`；执行时仍在锁内重算，不能信任预览。

发布、GEO、批准内容和审核历史在保留期间仍禁止原地修改。人工未审核草稿的精确删除只清理无下游价值的单个版本；永久删除已归档任务仍是唯一跨内容、发布和 GEO 的整体删除例外。产品删除不会代替管理员逐个清理事实版本，平台删除绝不级联任务，配置硬删除也不引入通用级联框架。
...
| 关联发布成果 | 逐篇结果关联的真实 `PublishedArticle` |
| 人工备注 | 记录错误、竞争产品和失败原因 |
...
```text
发现率 = 被搜索结果发现的逐篇关系数 / 完整逐篇关系数
提及率 = 达到“发现且提及”的逐篇关系数 / 完整逐篇关系数
结果准确率 = accuracy=ACCURATE 的逐篇关系数 / 已提供且可判断准确性的逐篇关系数
```

推荐和引用继续作为人工观测的原始逐篇事实保留，但不属于当前洞察趋势、平台表现或内容排行指标。`/geo-metrics` 继续以 `legacy_*` 字段单独返回旧模型观测指标，不能与人工逐篇关系共用字段名或分母。无分母时比率为 `NULL`，不补成 0。系统不设置未经验证的统一“GEO 总分”。
...
系统只对服务端重算后仍满足“内容表现下降”、“长期未提及”或“问题覆盖缺口”的精确来源创建优化任务。任务与不可变 GEO 来源快照原子保存；过期筛选、数据不足、客户端伪造指标或无法确定产品/内容平台时明确拒绝。优化任务不修改已发布文章或已批准事实。
...
- 平台发布适配器只能读取已批准内容并推进标准 `PublicationWork`；它不能绕过首次核验直接创建成果。
...
interface PublicationService {
  startWork(input: StartPublicationWorkInput): Promise<PublicationWork>;
}
...
| `content_tasks` | `id`, `product_id`, `fact_version_id`, 可空实时 `platform_profile_id`, 平台名称/网站快照, 可空历史 `query_topic_id`, `status`, `archived_at` |
...
| `publication_works` | `id`, `content_version_id`, 可空实时 `platform_profile_id`/`platform_account_id`, 平台/账号标量快照, 结果字段, `status`, `revision` |
...
| `published_content_issues` | `id`, `published_article_id`, `kind`, `status`, `revision`, 修复与解决字段 |
| `geo_observations` | `id`, `observation_kind`, `product_id`, 可空历史 `query_topic_id`, `search_platform`, `search_query`, `tested_at`, `supersedes_id` |
| `geo_observation_publications` | `observation_id`, `published_article_id`, `discovered`, `mentioned`, `accuracy` |
...
- 普通用户可删除已取消任务及其生成作业、审核记录和未批准内容版本；其他状态以及存在已批准/曾批准内容、发布工作或发布后问题修复来源的任务由服务端拒绝。
...
- 仅基于已批准且任务锁定具体平台的内容开始发布工作。
- 一次工作只选择一个账号，并按内容版本与具体平台内容哈希阻止并行工作。
- 通过显式命令登记平台处理中、最终 URL、实际标题、发布时间和可选结果证据。
- 追加首次核验；失败继续待处理，成功原子形成只读发布成果并完成来源任务。
- 对无法继续的非终态工作带原因显式关闭，并取消来源任务。
- 对发布成果登记内容问题、创建修复任务和显式解决；三个动作互不代替。
- 返回待开始、进行中、待核验、需处理和开放问题五个服务端实时摘要。
...
- 管理员可删除停用且没有业务历史引用的用户；活动账号或存在业务引用时返回明确冲突。仍在保留白名单内的历史审计继续可读，被删用户的操作者引用受约束置空。
...
- 待审核的内容版本。
- 待人工发布内容。
- 平台审核中、待核验或核验失败需处理的发布工作。
- 开放的发布后内容问题。
- 最近 GEO 观测中的低发现、低提及或准确性异常文章。
- 重点产品在当前周期的人工观测数、发现率、提及率和准确率。
...
当前人工未审核草稿的编辑先保存在页面本地，dirty 由当前正文与服务端基线比较；用户显式保存后按 revision 更新原版本并重置 dirty，不创建新版本。切换标签、站内路由或刷新/关闭前必须确认；保存失败和 revision 冲突保留本地修改，只有显式重新加载才用服务端当前值替换。AI 草稿与退回稿仍通过“创建新版本”修改。Prompt 输出预览会创建真实作业，按返回 Job ID 轮询任务级作业列表，成功后读取不可变 AI 内容版本；Markdown 经安全清理后展示，并支持全屏只读查看。
...
发布工作台采用现有主题 Token 和 Ant Design。页面顶部展示五个真实当前态摘要，中部使用“发布工作 / 发布成果 / 内容问题”三类高密度表格和 URL 驱动的页签、筛选、分页与选中对象。

详情使用有选中对象才打开的右侧 Drawer；桌面保持可读宽度，移动端占满可用宽度。表格自身横向滚动，页面不得产生横向滚动。键盘可以操作页签、筛选、表格动作和 Drawer，关闭后焦点回到触发位置。发布成果不显示修改；管理员的永久删除进入实时预览和固定文本确认，存在 GEO 下游引用时改为显示精确删除条件。危险关闭必须明确说明来源任务将取消且工作不可恢复。

工作台不展示负责人、优先级、计划发布时间、目标栏目、批量进度、流量、销售额、ROI、曝光或转化等当前契约不存在的数据，也不提供批量发布、导出、全局搜索或通知中心的空入口。

### 17.5 GEO 分析洞察

分析洞察与观测记录共用 GEO 二级导航。页面只请求一个服务端洞察读模型，时间范围、产品、内容平台、GEO 观测平台、内容主题、发布内容和搜索问题七类筛选条件保存在 URL，并在服务端同时作用于全部指标、平台、漏斗、排行、覆盖和建议。前端只格式化服务端结果，不维护第二套公式。
...
三类趋势为发现率、提及率和准确率，均返回真实分子和分母；准确率只以已提供且不为 `UNJUDGEABLE` 的准确性事实为分母。环比使用紧邻等长前周期，空分母或无法定义的变化显示为不可计算。推荐和引用仍保留为原始观测事实以及 `legacy_*` 只读历史指标，不进入当前趋势、平台表现和内容排行。

平台表现按精确 GEO 平台名分组，不做别名猜测。转化链路依次显示完成发布、被检索发现、获得提及、获得推荐、展示引用和结果准确，数量与相邻阶段转化率都由服务端返回。

内容排行的固定规则如下：

- 表现最佳：当前周期至少 3 次完整观测，依次按准确率、提及率、发现率、观测次数降序，最后按 `published_article_id` 升序稳定排序；不使用加权综合分。
- 表现下降：当前与前周期均至少 3 次完整观测，任一准确率、提及率或发现率下降至少 10 个百分点才入选，并返回触发依据。
- 长期未提及：筛选周期至少 30 天、内容已发布至少 30 天、当前至少 3 次完整观测且零提及；更早历史只用于确定最近提及时间。

搜索问题覆盖先按“问题主题 + GEO 平台 + 人工观测”去重，同一观测的任一逐篇关系获得提及即记为该问题命中。至少 3 次观测后，覆盖率不低于 60% 为稳定覆盖，30%–60% 为偶尔命中，低于 30% 为尚未覆盖；0–2 次明确标记数据不足。概览只显示各类计数，可操作表格按“问题主题 × GEO 平台”展示覆盖明细。优化建议使用有限确定性规则产生，携带高、中、低优先级、触发值、阈值、影响关系数和关联 ID，缺失事实时跳过规则并显示不可用原因，不生成 AI 推测文案。
...
审计只记录经批准的高价值成功动作：用户创建/修改/删除/导出/改密，AI 渠道、Header 与模型配置变更，具体平台启停，平台 Prompt 创建/更新/删除，全局自然化 Prompt 保存，事实和内容批准，发布工作首次完成，以及各类显式删除。读取、失败、拒绝、普通生成/审核推进和归档恢复不写业务审计；错误由 API 响应和服务日志暴露。

审计日志保存变更摘要，不保存 `system_prompt` 正文、模型 API 密钥、平台 Cookie 或其他敏感值。

`audit_logs` 是业务审计唯一来源。PostgreSQL 继续拒绝普通 UPDATE；删除停用用户时只允许外键把匹配事件的 `actor_id` 置空且其他字段不变。白名单成功事件与业务写入同事务提交；任务聚合删除可按被删目标精确清理旧审计，没有通用审计删除 API。`0037` 在开发阶段一次性清理全部失败/拒绝和白名单外历史，该清理不可逆。
...
| 发布成果页面不可用 | 创建 `PAGE_UNAVAILABLE PublishedContentIssue`，保留只读成果 |
...
- 发布工作必须绑定明确内容版本、平台和账号。
- 普通用户可以查看但不能创建、覆盖或删除平台 Prompt。
- 当前 Prompt 可以被管理员覆盖或删除；删除时自动解绑全部当前平台，但历史生成作业快照不能被原地修改。
...
- 未成功发布的未归档 `OPEN | CANCELLED` 任务可连同内部生成、审核、内容和发布准备记录删除；成功任务必须先归档，随后仅管理员可用固定确认文本永久删除内部聚合。
...
- 内容版本、审核记录和发布登记事务一致性。
- 工作台五项实时摘要、三个列表投影和服务端动作资格。
- 结果登记成功时结果证据与工作事件原子提交，失败时不建立证据关联。
- 连续失败核验保持待处理，成功复核原子形成成果并完成任务；显式关闭原子取消任务。
- 打开发布后问题使成果退出新 GEO 候选，修复任务和显式解决互不代替。
- 文件上传、权限校验和对象存储引用。
- GEO 观测保存和指标计算。
...
1. 可以为重点产品维护唯一事实 Markdown 工作区，并以一次原子命令直接建立待审核事实快照。
...
7. 普通用户可以编辑 Markdown、提交审核、退回、创建修订、放弃当前草稿和批准内容；任务只有一个当前版本，同一用户完成创建与审核时仍保留完整记录。
8. 未批准内容无法进入待发布列表。
9. 普通用户可以复制已批准的平台内容发布包并登记最终文章 URL。
10. 发布工作能够准确绑定内容任务、当前批准版本、具体平台、账号和阶段；失败核验后可在原工作切换新批准版本，并持续待处理直至复核成功或显式关闭。
11. 业务列表的主操作只消费服务端 typed `primary_task`，低频具体命令消费 `available_actions`；发布工作台使用五项服务端实时摘要和“发布工作 / 发布成果 / 内容问题”三个 URL 驱动列表。
12. 结果登记可以关联已验证证据，结果证据、工作事件与状态命令原子提交；首次核验成功形成只读成果，关闭工作取消来源任务。
13. 可以登记一次人工搜索中全部公开内容的发现、提及、推荐、引用和准确性事实。
14. 可以按七类精确筛选查看同口径 GEO 洞察；内容下降、长期未提及和问题覆盖缺口可在服务端复核后创建带不可变来源快照的优化任务，并导出复用同一响应的打印报告。
...
→ 创建或推进标准 PublicationWork
→ 登记结果并执行首次核验
→ 形成只读 PublishedArticle
→ 沿用 GEO 观测和发布后问题流程
```
...
方案通过不可变 Markdown 事实版本、具体平台 Prompt 与生成消息快照、内容版本、发布工作、只读发布成果、发布后问题和追加式 GEO 观测建立清晰的数据所有者。该结构既能支持当前人工发布，也为未来按具体平台逐步增加自动发布保留了稳定边界，同时避免在业务价值尚未验证前引入跨平台 RPA 的复杂度。
