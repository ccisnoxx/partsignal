...
export function ProductFactsPage() {
  ...
  const reviewContext = useQuery({ queryKey: ['fact-review-context', reviewTarget?.id], queryFn: async () => unwrap(await api.GET('/api/v1/fact-versions/{fact_version_id}/review-context', { params: { path: { fact_version_id: reviewTarget?.id ?? '' } } })), enabled: !!reviewTarget });
  ...
  return (
    <div className="page-stack">
      ...
      <PageHeader eyebrow="PRODUCT FACT WORKSPACE" title={<span className="data-code">{product.data.part_number}</span>} description={`${product.data.brand} · ${product.data.category} · 工作区修订 ${draft.data.revision}`} breadcrumbs={[{ title: <Link to="/products">产品事实</Link> }, { title: product.data.part_number }]} actions={<StatusTag status={product.data.status} />} />
