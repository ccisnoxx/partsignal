...
| 系统管理员 | 系统维护人员 | 拥有普通用户的业务能力，并额外管理用户、平台类型与 Prompt、AI 渠道、Header、模型和系统安全配置 |
...
系统只保存 `ADMIN` / `ENGINEER` 账号类型和启停状态，不建设角色表、权限配置界面或自定义权限引擎。所有写操作都必须记录实际操作者，并在服务端同时校验账号状态、管理员能力和业务状态转换。普通用户可以只读查看任务平台类型当前使用的完整 Prompt Markdown，但不能创建、覆盖或删除 Prompt，也不能管理渠道和模型。
...
flowchart TD
    ...
    C -->|退回| A
    C -->|批准| D[创建内容任务]
    D --> E[锁定平台类型并查看当前 Prompt]
    E --> F[选择模型并锁定完整输入快照]
    F --> G[生成平台差异化 Markdown 草稿]
    G --> H[人工编辑]
    H --> I{内容审核}
    I -->|退回| H
    I -->|批准| J[待人工发布]
...
1. 普通用户创建或更新公司产品、进口参考型号和替代关系。
2. 为关键参数和替代结论关联数据手册、测试报告或应用案例。
3. 系统生成新的事实版本草稿。
4. 普通用户核对参数、测试条件、证据和替代等级，并显式执行批准或退回操作；系统不以审核人与创建人是否相同作为阻断条件。
5. 审核通过后，事实版本变为不可变的已批准快照。
6. 后续修改必须创建新版本，不允许直接修改已批准版本。

### 7.2 内容生成流程

1. 普通用户创建内容任务，选择目标问题、产品和具体平台；任务创建时冻结平台类型身份，不允许生成时重新选择平台。
2. 用户在任务中维护本次生成所需的产品描述、参数、特性和产品对比等专业 Markdown 输入，任务绑定的已批准事实继续作为只读权威来源。
3. 用户查看该平台类型当前唯一的 Prompt Markdown，并从管理员已启用的模型中选择本次生成模型。
4. 服务端拼接固定安全契约和当前平台 Prompt 作为 `system` 消息，以工程师输入、已批准事实和任务要求构造 `user` 消息。
5. 创建 `GenerationJob` 时冻结渠道非敏感信息、模型参数、最终 system/user message、事实和任务要求，再异步调用模型生成结构化 Markdown 草稿。
6. 系统保存不可变作业快照、供应商请求 ID、耗时和供应商实际返回的 token 用量；缺失用量保持为空，不估算、不补零。
7. 普通用户编辑 Markdown 正文，形成新的内容版本，并核对事实准确性、表达合规性和平台适配性后确认审核结果。
...
| `PlatformType` | 平台类型 | 归类问答、论坛、行业媒体等内容形态 |
...
| `GenerationJob` | 生成作业 | 冻结一次生成的完整非敏感输入并保存执行状态和指标 |
...
erDiagram
    ...
    PLATFORM_TYPE ||--o{ CONTENT_TASK : snapshotted_by
...
`ContentTask` 创建时锁定具体平台和平台类型，后续可以继续修改任务级 `user_prompt_markdown`。每次生成时单独选择一个已启用模型；`GenerationJob.input_snapshot` 才是某次实际模型调用上下文的权威记录，不能用任务或配置后来更新的当前值解释历史内容。

#### `PlatformType` 与 `PlatformPrompt`

`PlatformType` 表示问答、技术论坛、行业媒体等稳定内容形态。每个平台类型最多拥有一条 `PlatformPrompt`，正文就是一份普通 Markdown 文档，不支持变量、循环、条件或 Prompt 编排。建议保存：

- 平台类型名称和唯一 `slug`。
- 当前唯一 `template_markdown`、修订号、维护人和维护时间。
- 具体平台到平台类型的归类关系。

只有管理员可以创建、覆盖和物理删除当前 Prompt。保存时原地覆盖并立即影响新作业，不保存配置版本历史；普通用户在任务页只读查看完整 Markdown。配置修改或删除不影响既有生成，因为 `GenerationJob.input_snapshot` 保存当时最终拼接的 system message。
...
- 发布账号必须属于内容任务锁定的平台；前端只展示匹配账号，服务端和 PostgreSQL 共同拒绝错绑。
...
`COMPLETED` 记录任务曾完成发布闭环，是不可回退的历史终态。任务存在待人工发布、平台审核中或已发布记录时不能取消。具体进度通过事实版本、内容版本、发布记录和发布异常待办推导，避免维护重复状态。
...
flowchart LR
    ...
    A --> C[按平台类型加载当前 PlatformPrompt]
    A --> D[选择已测试并启用的 AIModel]
    B --> E[构造 user 消息]
    A --> E
    C --> F[拼接固定契约与平台 Prompt]
    D --> G[创建不可变 GenerationJob 快照]
    E --> G
    F --> G
    G --> H[调用 Chat Completions 并严格解析]
    H --> I[自动质量检查]
    I --> J[保存 DRAFT ContentVersion]
```
...
具体平台归属于一个 `PlatformType`。平台规则仍使用不可变版本约束标题、正文长度、外链、表格、联系方式、语气和禁用表达；平台类型拥有当前唯一的 Prompt Markdown。具体平台重新归类只影响后续创建的内容任务，既有任务保存平台类型快照。

### 10.4 平台 Prompt

平台 Prompt 由管理员维护，用于规定某类内容的生成方式。例如问答平台使用工程解答 Prompt，技术论坛使用经验分享 Prompt，行业媒体使用产品对比 Prompt。每个平台类型最多一份 Prompt，正文是一篇可全选、复制、粘贴和修改的普通 Markdown，不需要理解模板语法。
...
├── content-planning    目标问题、平台规则、平台类型和内容任务
├── configuration       平台 Prompt、AI 渠道、Header 和模型
...
| `platform_profiles` | `id`, `platform_type_id`, `name`, `rules_json`, `revision` |
...
| `content_tasks` | `id`, `product_id`, `fact_version_id`, `query_topic_id`, `platform_profile_version_id`, `platform_type_snapshot`, `user_prompt_markdown`, `status` |
...
| `content_versions` | `id`, `task_id`, `source_job_id`, `version`, `title`, `summary`, `body_markdown`, `tags`, `status`, `content_hash` |
...
- 创建内容任务。
- 普通用户保存任务级 Markdown `user_prompt`，只读查看平台类型当前完整 Prompt。
- 管理员维护平台类型、当前唯一 Prompt、AI 渠道、渠道 Header 和模型。
- 管理员发现或手工添加模型，完成严格连接测试并启停渠道和模型。
- 普通用户选择一个已启用模型，基于事实版本、工程师输入和当前平台 Prompt 创建生成作业。
...
所有写操作在服务端再次校验账号状态、管理员能力和业务状态，不依赖前端隐藏按钮保证业务规则。Prompt 与 AI 配置写接口只允许管理员访问；普通用户可以读取任务所用 Prompt，但不能提交或修改 `system_prompt` 正文。
...
- 左侧：目标问题、锁定平台、只读 Prompt、模型选择、工程师输入和内容任务。
- 中间：标题、摘要和 Markdown 正文编辑。
- 右侧：产品事实、证据、替代边界、生成输入快照和质量警告。

应支持查看 AI 原始版本与当前版本差异，并明确显示当前绑定的事实版本、完整 system/user message 快照、渠道、模型和参数。普通用户可以查看当前平台 Prompt Markdown，但不能修改。

“系统设置”仅对管理员显示并由服务端限制访问；普通用户通过内容任务页面选择已启用的平台和 Prompt，不进入配置页面。
...
- 发布登记、URL 变更、拒绝和下线。
- GEO 观测创建和人工准确性判断。
- 平台 Prompt 创建、覆盖和物理删除，AI 渠道、Header、模型测试及启停。
- 用户账号、管理员标识和关键系统配置变更。
...
| 任务平台类型缺少当前 Prompt | 阻止生成，要求管理员创建 Prompt |
...
- 事实版本创建、审核和冻结。
- 平台 Prompt 覆盖/删除、普通用户只读查看和管理员权限校验。
...
```text
管理员配置平台类型、Prompt、渠道 Header 和模型
→ 模型严格测试通过并启用
→ 录入产品事实
→ 批准事实版本
→ 创建论坛平台内容任务并填写工程师输入
...
1. 产品、参考型号和替代关系管理。
2. 证据上传和事实版本审核。
3. 目标问题、平台类型、平台规则和当前唯一 Prompt Markdown 管理。
4. 内容任务、生成输入快照、AI 生成、Markdown 人工编辑和版本管理。
5. 内容审核和审核记录。
...
截至提交 `d22d401` 和 `cebb0d0`，阶段一至阶段四的 MVP 纵向闭环、配置中心、旧初始化账号精简和显式改密入口均已完成并通过自动化验收。当前进入上线可靠性和生产边界复核；阶段五仍属于未来评估范围。
...
- 实现问题库、平台类型、平台配置、当前 Prompt 和内容任务。
...
3. 管理员可以维护平台类型和当前唯一 Prompt Markdown，普通用户只能只读查看；Prompt 覆盖或删除不改变历史作业快照。
4. 管理员可以配置至少两个 OpenAI-compatible 渠道或模型；`/models` 失败时仍能手工添加 `model_id`，至少一个模型测试成功后才能启用渠道，每个模型仍需分别测试成功。普通用户每次生成选择并锁定一个模型。
5. 生成内容能够追溯到事实版本、完整 system/user message 快照、渠道、模型和生成参数。
6. 普通用户可以编辑 Markdown、提交审核、退回和批准内容，同一用户完成创建与审核时仍保留完整记录。
7. 未批准内容无法进入待发布列表。
8. 普通用户可以复制发布包并登记最终文章 URL。
9. 发布记录能够准确绑定内容版本、平台、账号和状态。
10. 可以登记一次模型测试的提及、推荐、引用和准确性结果。
11. 可以按产品、问题、模型和时间查看基础指标。
12. 关键事实、Prompt 管理、生成、审核和发布操作均有审计记录。
13. 完整主流程有自动化测试或明确的可重复验收脚本。
14. API Key 和敏感 Header 使用数据库密文且永不回显，生产环境拒绝非公网 HTTPS、重定向和保留 Header 覆盖。
15. 空数据库初始化得到一个管理员和一个强制首次改密的普通用户，管理员可以新增、停用、启用和重置其他用户密码，并能修改自己的密码。
16. AI 协议只调用带版本前缀 `base_url` 下的 `GET /models` 和 `POST /chat/completions`，不调用 Responses API，也不自动探测其他路径。
...
5. 第一批平台类型、Prompt Markdown 正文和管理员维护责任人。
6. 可发送给第三方大模型的数据范围。
