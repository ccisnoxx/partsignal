...
export function PlatformsPage() {
  ...
  const versions = useQuery({ queryKey: ['platform-profile-versions', manageProfile?.id], queryFn: async () => unwrap(await api.GET('/api/v1/platform-profiles/{platform_profile_id}/versions', { params: { path: { platform_profile_id: manageProfile?.id ?? '' } } })), enabled: !!manageProfile, staleTime: QUERY_STALE_TIME.configuration });
