...
export function SettingsPage() {
  return <div className="page-stack"><PageHeader eyebrow="WORKSPACE" title="业务设置" description="维护目标问题和公开平台账号标识；用户与 AI 配置由管理员在独立入口管理。" />
    <Tabs items={[
      { key: 'topics', label: '目标问题', children: <TopicsPanel /> },
      { key: 'accounts', label: '平台账号标识', children: <PlatformAccountsPanel /> },
    ]} />
  </div>;
}
...
function TopicsPanel() {
  ...
  return <Card extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>新增问题</Button>}>
    ...
    <TableRegion label="目标问题列表"><Table<QueryTopic> rowKey="id" loading={topics.isLoading} dataSource={topics.data?.items} scroll={{ x: 720 }} columns={[{ title: '标准问题', dataIndex: 'canonical_question' }, { title: '意图', dataIndex: 'intent_type', render: (value) => <span className="data-code">{value}</span> }, { title: '变体', dataIndex: 'variants', render: (items: string[]) => items.join(' / ') }]} /></TableRegion>
...
export function PlatformsPanel() {
  ...
  return <Card extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>新增平台</Button>}>
    ...
    <TableRegion label="平台规则列表"><Table<PlatformProfile> rowKey="id" loading={platforms.isLoading} dataSource={platforms.data?.items} scroll={{ x: 980 }} columns={[
...
export function AuditPanel() {
  ...
  return <Card>{audit.error && <Alert role="alert" type="error" showIcon message={errorMessage(audit.error)} />}<TableRegion label="审计日志"><Table<Schema<'AuditLog'>> rowKey="id" loading={audit.isLoading} dataSource={audit.data?.items} scroll={{ x: 900 }} columns={[{ title: '时间', dataIndex: 'created_at', render: (value) => <span className="data-code">{new Date(value).toLocaleString('zh-CN')}</span> }, { title: '动作', dataIndex: 'action' }, { title: '对象', render: (_, row) => <span className="data-code">{row.target_type} / {row.target_id}</span> }, { title: '请求 ID', dataIndex: 'request_id', render: (value) => <span className="data-code">{value}</span> }]} /></TableRegion></Card>;
}
