...
test('URL 支持 direct、refresh、status、pagination 与 Back/Forward，并归一非法参数', async ({ page, publicationApi }) => {
  await page.goto('/publishing/work?page=2&pageSize=10');
  await expect(page.getByRole('heading', { name: '发布工作' })).toBeVisible();
