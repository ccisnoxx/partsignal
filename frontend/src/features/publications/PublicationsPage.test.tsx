...
test('候选登记抽屉只展示匹配账号并在放弃未提交内容前确认', async () => {
  ...
  fireEvent.click(await screen.findByText('匹配账号 / matched'));
...
test('登记失败保留输入与未绑定证据，重试成功仍携带结果证据 ID', async () => {
  ...
  fireEvent.click(await within(dialog).findByRole('button', { name: '登记已发布' }));
  fireEvent.click(within(dialog).getByRole('button', { name: '选择测试证据' }));
  expect(await screen.findByText('published.png')).toBeInTheDocument();
  expect(screen.getByText('已上传，尚未绑定')).toBeInTheDocument();
  fireEvent.change(screen.getByRole('textbox', { name: '实际发布标题' }), { target: { value: content.title } });
  fireEvent.change(screen.getByRole('textbox', { name: '最终文章 URL' }), { target: { value: 'https://community.example.invalid/post/1' } });
  fireEvent.change(screen.getByRole('textbox', { name: '发布时间' }), { target: { value: '2026-07-20T10:00:00+08:00' } });
  fireEvent.change(screen.getByRole('textbox', { name: '操作说明' }), { target: { value: '人工发布完成' } });
