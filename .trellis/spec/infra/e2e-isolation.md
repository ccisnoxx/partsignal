...
本地或 CI 运行根 `make e2e` 时适用。V1 与 V2 Product Facts/Content Editor 真实栈 Playwright 使用真实
...
- 根 `make e2e` 先通过 `e2e-local.sh` 在同一隔离栈运行 V2 Product Facts 与 Content Editor 真实 flow，再运行 V1 suite；成功并清理后运行 V2 fixture-based 页面 suite，任一阶段失败时根 target 非零。
- V2 Product Facts gate 必须早于 V1 suite；否则既有 V1 失败会在 `set -e` 下跳过本 Task 新增的真实闭环门禁。
- V2 `webServer` 必须执行 production build 后通过 `vite preview` 服务当前 artifact，不使用 Vite dev server。
- `deploy/scripts/e2e-local.sh` 必须在同一隔离数据库生命周期内以 `VITE_API_BASE_URL` 构建并启动 V2 production preview，再运行 Product Facts 真实栈 spec；不得另建数据库、seed 或清理入口。
- V2 真实栈 spec 只允许测试 API 登录、读取最终投影和 V2 尚无页面的最小前置配置；Product Facts、ContentTask 创建及 Editor manual/save/submit 等业务 mutation 必须通过 V2 页面，禁止 `page.route`、`route.fulfill` 或固定成功状态。
...
- Good：V2 Product Facts 与 V1 在同一独立数据库栈依次完成真实 flow 并精确清理，随后 V2 fixture suite 对 production artifact 完成页面矩阵。
...
- `deploy/scripts/e2e-local.sh` 必须实际运行 `tests/e2e/product-facts-real-stack.spec.ts --project=foundation-desktop`，覆盖批准交接、退回修订和独立 Content Editor 人工首稿 → 保存 → 提交三条真实 flow。
...
Correct: 先确认 5173 与 Redis broker 逻辑库由本次 E2E 独占 → allowlist 数据库与 mktemp 精确清理 → V2 Product Facts gate → V1 suite → 清理后运行 V2 fixture suite → 任一失败使根入口失败
```
