...
- 首次成功核验原子形成只读发布成果并完成来源内容任务；显式关闭原子取消来源任务。成功成果不因后续页面问题自动回退；只有管理员显式永久删除无 GEO 下游引用的完整成果聚合时，来源任务才恢复为 `OPEN`。
...
`COMPLETED` 记录任务已完成首次核验闭环，是不可回退的业务终态。`archived_at` 与该状态正交：归档只改变默认可见性，恢复不会把任务改回 `OPEN`。非终态发布工作存在时不能直接取消任务；关闭发布工作会在同一事务把来源任务置为 `CANCELLED`。生成和发布进度通过作业、内容版本与发布工作推导，不维护重复状态。
