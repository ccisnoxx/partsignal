...
Correction Workspace required evidence 分为三层：PostgreSQL integration 证明权限、历史 ID→当前尾、候选新增/退出、历史空 Topic、append-only POST、冻结字段、证据不可复用、冲突无半成品及原链不变；并以 current-head catalog/真实 duplicate INSERT 证明 successor partial unique exact diagnostics，以合规双 Session 证明 loser 等待 production `FOR UPDATE` 后走 precheck，以 test-only race 证明 loser 等待真实 `INSERT INTO geo_observations` 后收到 exact `23505`。两条路径都必须只留下一个 successor，precheck/exact 四元组相同；known mapper rollback 后原 Session 可立即复用，unknown 由 caller rollback 后复用且 HTTP 只冻结 500 no-leak。API/model/page tests 证明 generated context、表单边界、权威 `supersedes_id`、只提交新证据、pending 防重、`GEO_PUBLICATIONS_CHANGED`、`GEO_OBSERVATION_HAS_SUCCESSOR` 两类 stale 409 不 replay；`GEO_OBSERVATION_CONTEXT_INCOMPLETE` 整体 blocked、显式 GET 重读且不返回部分链；显式刷新按文章 ID 合并及 canonical replace。production-artifact strict fixture 只允许 auth、correction context、通用 POST、三阶段上传和精确 Detail GET，覆盖 Detail 入口/direct/refresh、404/403/Legacy、权限变化、上传 complete 重试、DirtyGuard、焦点及 375/768/1024/1440 无根级溢出。

T5-I6 后端门禁逐项覆盖剩余 11 个非 revision GEO producer：前 10 个 exact `409 GEO_OBSERVATION_CONTEXT_INCOMPLETE`，锁后链集合变化 exact `409 GEO_OBSERVATION_CHAIN_CHANGED`，均保留场景 message、`details={}` 与 body/header/inbound request ID。Detail/Correction 不返回部分历史；create evidence 缺失零写入；GEO delete 与三个共享 content-task operation 失败不得留下链/聚合/relation/file cleanup 或 SUCCESS AuditLog。branch/cycle/identity/incomplete 使用事务内受控 fixture，锁后成员变化须真实 PostgreSQL interleaving；finally 与 fresh connection 证明 index/trigger 恢复。unknown 500 只验 no-leak，不映射 409；current-head `0037` GEO append-only trigger 为 UPDATE-only，不把 `0029` 的历史 DELETE guard 当现行约束。

T6-G 页面测试分别证明 context blocked、cached Detail 动作冻结、chain changed 的刷新/重开/重新确认与 no replay。T6-C 的 content-task list/detail unit 与 E2E 测试分别证明 preview GET 失败后旧预览失效、普通 DELETE blocked/no replay、permanent-delete POST 失败后旧预览和确认文本失效并要求显式刷新/重开/重新确认。两个前端 slice 完成前，T5-I6 backend code 不得部署或发布；不解析 message，不建立第二套 code registry。
...
`tests/e2e/geo-observation-correction.spec.ts` 使用独立 generated-type `geo-correction.fixture.ts`，只开放认证、Manual Detail 入口、correction-context、通用 Observation POST、三阶段文件上传和创建 ID 对应的精确 Detail GET；未声明 API 在 teardown 失败。后端 integration 与 contract tests 覆盖 append-only、服务端资格/尾/候选/冻结字段/证据边界，以及 successor precheck/exact mapping、rollback、原子性、request ID 与 unknown no-leak；API/model/page tests 覆盖严格上下文、初值、payload、错误映射、pending 防重、缓存失效、DirtyGuard 和冲突合并。production-artifact E2E 覆盖历史 ID canonical replace、完整只读历史、上传 complete 失败/重试、CSRF、无 Idempotency-Key、三类 canonical-context stale 409 不 replay、显式刷新保留草稿/Evidence/request ID 并采用新尾、404/403/Legacy/提交时权限变化、响应 ID handoff、键盘与 375/768/1024/1440 页面根无横向溢出。T5-I5 只增加后端证据；successor 页面分支由 T6 增加，T6 完成前不得单独发布 T5-I5 server code。fixture 不代表 GEO 完整 real-stack 闭环。
