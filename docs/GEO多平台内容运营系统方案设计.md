...
6. 每个发布工作必须绑定唯一的已批准内容版本，并只选择一个属于任务平台的账号。
...
5. 人工核对页面正文与批准版本一致后执行首次核验。失败时追加快照并进入 `ACTION_REQUIRED`，修正登记结果后继续复核；成功时原子形成只读 `PublishedArticle` 并完成来源内容任务。
...
`PublicationWork` 保存锁定内容版本、内容哈希、任务平台下的一个账号、目标栏目、结果字段、当前阶段、revision、关闭事实和创建人。每次状态变化追加 `PublicationWorkEvent`；每次核验追加当时实际标题、最终 URL、发布时间、结论、说明、操作者和时间快照。发布证据通过追加式 `publication_attachments` 关联，只接受已验证的 `OPERATION_SCREENSHOT`。

失败核验不会产生新工作或终止来源任务，而是把同一工作置为 `ACTION_REQUIRED`。用户修正实际标题、URL 或发布时间后重新进入待核验；首次通过时创建与工作同 ID 的 `PublishedArticle`，它只读投影内容版本、平台、账号、实际标题、最终 URL、发布时间和首次成功核验。工作与来源任务同时进入 `COMPLETED`。
...
服务端以独立核验命令保存 `PASSED | FAILED`、`content_matches`、说明及当时结果快照。`FAILED` 必须说明原因，工作进入 `ACTION_REQUIRED` 并继续待处理；`PASSED` 必须满足 `content_matches=true`，原子形成发布成果并完成来源任务。自动页面抓取不在当前范围。
...
10. 发布工作能够准确绑定内容版本、任务具体平台、账号和阶段，失败核验持续待处理直至复核成功或显式关闭。
11. 发布工作台使用五项服务端实时摘要和“发布工作 / 发布成果 / 内容问题”三个 URL 驱动列表；所有动作只来自服务端投影。
