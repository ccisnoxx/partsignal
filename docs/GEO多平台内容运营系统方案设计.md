...
> 文档版本：V1.6
> 编制日期：2026-07-20
> 当前阶段：MVP 已实现，人工发布工作台与发布异常闭环已完成
...
2. 登记人工搜索平台、实际搜索词、测试时间和真实结果截图。
3. 系统列出该产品当前具有公开链接的全部已发布文章。
4. 操作人员逐篇标记“已推荐”或“未推荐”，不得漏标或由系统猜测。
5. 系统追加保存观测、截图和逐篇结果，不调用模型联网搜索，也不覆盖历史。
6. 后续根据按产品和文章复算的结果创建新内容任务或修改建议。
...
- 被核对的产品、人工搜索网站和实际搜索词。
- 测试时间、测试人员和补充说明。
- 该产品当前全部可观测文章的逐篇已推荐/未推荐结论。
- 至少一张已验证的搜索结果截图。

旧模型观测继续保留原目标问题、模型、联网标识、回答摘要、引用和准确性等历史字段，但不再用于创建新的人工观测。人工更正通过追加同产品新记录完成，不覆盖原记录。
...
当前 GEO 观测以产品、人工搜索网站、实际搜索词和已发布文章为维度。操作人员在站外网站实际搜索后登记证据，系统不维护固定搜索网站枚举，也不自动联网复查。

`QueryTopic` 继续保存历史内容任务和 `LEGACY_MODEL_RESULT` 观测使用的标准问题与自然语言变体；新内容任务和 `MANUAL_ARTICLE_SEARCH` 观测不再创建该关联。
...
| `geo_observation_publications` | `observation_id`, `publication_record_id`, `recommendation_status` |
...
运营界面统一使用中文页眉、状态、枚举、字段、提示和错误；`model_id`、API Key、URL、Markdown、JSON、Header、Prompt 和机器值保持原样。AI 渠道列表通过一次请求返回各渠道全部已启用模型摘要，卡片直接展示显示名与 `model_id`，不得逐卡请求形成 N+1。
...
- 事实版本创建、审核和冻结。
- 动态平台类型、具体平台 Prompt 覆盖、规则版本生命周期、普通用户只读查看和管理员权限校验。
...
截至 2026-07-16，MVP 纵向闭环、配置中心、账号治理、平台级 Prompt、受约束物理删除和运营界面中文化均已完成；阶段五仍属于未来评估范围。
...
13. 可以登记一次模型测试的提及、推荐、引用和准确性结果。
14. 可以按产品、问题、模型和时间查看基础指标。
...
18. 空数据库初始化得到一个管理员和一个强制首次改密的普通用户，管理员可以新增、停用、启用和重置其他用户密码，并能修改自己的密码。
