...
| 后端集成测试 | pytest + PostgreSQL | Prompt 所有权迁移、规则草稿与事实审核清理触发器、AI 渠道身份/列表/统计/审计、`0024` 审计回填与降级门禁、受约束删除、生成作业、发布约束和 OSS 适配器 |
| 前端单元测试 | Vitest + Testing Library | 平台可用性、AI 渠道三栏 URL 状态与敏感 mutation 清理、审计筛选/URL/刷新/详情/脱敏展示、中文枚举、表单、审核、编辑和异常状态 |
