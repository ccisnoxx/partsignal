...
export function ChangePasswordPage() {
  ...
  return <main className="centered"><Card style={{ width: 440 }}><Typography.Title level={2}>修改密码</Typography.Title><Typography.Paragraph type="secondary">{auth.user?.must_change_password ? '当前使用临时密码，完成修改后才能进入工作台。' : '修改后其他登录会话会被撤销。'}</Typography.Paragraph>{change.error && <Alert type="error" message={errorMessage(change.error)} />}<Form<Schema<'ChangePasswordRequest'>> layout="vertical" onFinish={(body) => change.mutate(body)}><Form.Item name="old_password" label="当前密码" rules={[{ required: true, min: 8 }]}><Input.Password autoComplete="current-password" /></Form.Item><Form.Item name="new_password" label="新密码" rules={[{ required: true, min: 12 }]}><Input.Password autoComplete="new-password" /></Form.Item><Button type="primary" htmlType="submit" block loading={change.isPending}>更新密码</Button></Form></Card></main>;
}
