      ...
      <Card title="发布记录">
        <Descriptions
          ...
          items={[
            ...
            { label: '内容任务', children: <Link to={`/tasks/${record.task_id}`}>{record.task_id}</Link> },
            { label: '内容版本', children: record.content_version_id },
            { label: '栏目', children: record.section_url },
            { label: '最终 URL', children: record.final_url ?? '—' },
            { label: '内容哈希', children: record.content_hash },
          ]}
        />
