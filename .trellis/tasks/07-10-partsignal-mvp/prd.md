...
- MVP 使用内部账号、PostgreSQL 会话、CSRF 和 `ADMIN` / `ENGINEER` 两种账号类型；事实与内容允许创建者显式自审，但仍保留状态机、质量门禁和审计。
...
- 真实大模型通过配置中心接入 OpenAI-compatible Chat Completions；确定性生成器只用于本地开发和自动化测试，不能掩盖真实调用失败。生产 OSS、生产 VPS 和外部发布平台仍不在本次实现范围。

## Acceptance Criteria

- [ ] `make dev` 可启动前端、API、Worker、PostgreSQL 和 Redis，数据库迁移成功。
- [ ] 用户可登录，`ADMIN` / `ENGINEER` 权限、自审状态转换和审计由服务端执行。
- [ ] 可录入产品、参考型号、参数、替代关系和证据，并批准不可变 `FactVersion`。
- [ ] 可创建问题、平台配置和绑定已批准事实的 `ContentTask`。
- [ ] Celery 可基于任务锁定事实、工程师 `user_prompt`、平台 Prompt 和所选 OpenAI-compatible 模型创建且只创建一个 `DRAFT ContentVersion`；开发测试继续使用显式确定性生成器。
...
- 自动 GEO 模型监测和自动替代结论；非 OpenAI-compatible 私有协议、多阶段 Prompt 编排及模型自动降级。
