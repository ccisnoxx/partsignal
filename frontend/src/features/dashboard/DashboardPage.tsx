...
const shortcuts = [
  ...
  { label: '发布管理', description: '登记并跟进发布', to: '/publications', icon: <RocketOutlined /> },
...
export function DashboardPage() {
  ...
  const articleDiscoveryRate = rate(metrics.data?.article_discovery_rate);
      ...
      {summary.isLoading || metrics.isLoading ? (
        ...
        <>
          <section className="dashboard-kpi-grid" aria-label="GEO 管理指标">
            <MetricTile icon={<EyeOutlined />} label="人工观测" value={metrics.data?.manual_observation_count ?? 0} meta={`${metrics.data?.article_result_count ?? 0} 条逐篇结果`} />
            <MetricTile icon={<SearchOutlined />} label="文章发现率" value={articleDiscoveryRate ?? '—'} unit={articleDiscoveryRate == null ? undefined : '%'} percent={articleDiscoveryRate} meta={articleDiscoveryRate == null ? '暂无文章结果' : '已发现 / 全部文章结果'} tone="data" />
            <MetricTile icon={<FileTextOutlined />} label="文章提及率" value={articleMentionRate ?? '—'} unit={articleMentionRate == null ? undefined : '%'} percent={articleMentionRate} meta={articleMentionRate == null ? '暂无文章结果' : '已提及 / 全部文章结果'} tone="data" />
            <MetricTile icon={<CheckCircleOutlined />} label="文章准确率" value={articleAccuracyRate ?? '—'} unit={articleAccuracyRate == null ? undefined : '%'} percent={articleAccuracyRate} meta={articleAccuracyRate == null ? '暂无可判断结果' : '准确 / 可判断结果'} tone="data" />
          </section>
          ...
          <Card title="运营状态摘要" className="dashboard-glass-panel dashboard-status-panel">
            <div className="dashboard-status-grid">
              ...
              <section className={`dashboard-status-item dashboard-status-${geoNeedsAttention ? 'danger' : 'success'}`} aria-label="GEO 观测状态">
                ...
                <div className="dashboard-status-copy"><Typography.Text strong>GEO 观测</Typography.Text><Badge status={geoNeedsAttention ? 'error' : 'success'} text={geoNeedsAttention ? '需关注' : '正常'} /><Typography.Text type="secondary">发现率 {articleDiscoveryRate == null ? '—' : `${articleDiscoveryRate}%`} · 提及率 {articleMentionRate == null ? '—' : `${articleMentionRate}%`} · 准确性问题 {summary.data?.recent_accuracy_errors ?? 0}</Typography.Text></div>
              </section>
            </div>
          </Card>
