...
type Scenario = 'dashboard' | 'error' | 'facts' | 'task' | 'review' | 'publications' | 'publication-empty' | 'geo';
...
async function capture(page: Page, scenario: Scenario, path: string, readyText: string) {
  ...
  for (const viewport of viewports) {
    ...
    await expect(page).toHaveScreenshot(`${scenario}-${viewport.name}.png`, { fullPage: true });
