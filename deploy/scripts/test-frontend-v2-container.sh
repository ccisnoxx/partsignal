...
image=${PARTSIGNAL_FRONTEND_V2_TEST_IMAGE:-partsignal-frontend-v2:test}
container="partsignal-frontend-v2-test-$$"
test_dir=$(mktemp -d "${TMPDIR:-/tmp}/partsignal-frontend-v2-test.XXXXXX")

cleanup() {
  docker rm -f "$container" >/dev/null 2>&1 || true
  case "$test_dir" in
    "${TMPDIR:-/tmp}"/partsignal-frontend-v2-test.*) rm -rf "$test_dir" ;;
  esac
}
...
printf '%s\n' "Frontend V2 容器 fallback、缓存和 source map 自检通过"
