...
集合视图以 URL 作为可恢复状态源：产品使用 `q/page`，内容任务和 GEO 观测使用 `page`，人工发布使用 `tab` 与三个列表各自的页码，用户管理使用 `q/account_type/status/page/page_size`。用户状态选择器和“显示停用账号”开关投影同一个状态：默认只查启用账号，`status=DISABLED` 只查停用账号，`status=ALL` 查询全部。查询参数采用严格正整数与冻结枚举校验；无效值以 replace 回到默认值，Table、Tabs、筛选器和 Switch 不保留第二份已提交状态。
...
侧栏“业务设置”包含发布账号和仅管理员可见的用户管理；“GEO 问题库”与观测记录、分析洞察同属“GEO 观测”。审计日志位于仅管理员可见的“审计与安全”，继续使用一级路由 `/audit`，页面和服务端接口均执行管理员权限校验。
