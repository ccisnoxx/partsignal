...
同一浮层内的条件动作区关闭时，焦点所有权由动作入口决定：入口仍挂载则在清理条件状态后立即调用 `restoreFocus`；入口位于浮层外且关闭动作同时结束整个浮层链时，只调用外层关闭回调，由既有浮层关闭生命周期恢复外部触发器。不得让即将卸载的取消按钮成为最终焦点，也不得让内外两层同时执行回焦。
...
- 使用 Drawer `afterOpenChange(false)` 恢复焦点时，承载该回调的组件实例必须存活到关闭完成；按对象重置表单的动态 key 只能放在 Drawer 内容边界，不能让外层 key 随查询参数清除而提前替换生命周期所有者。
