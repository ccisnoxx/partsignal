...
- [x] `main` 非强制推送成功，`HEAD == origin/main`，发布包来自该权威提交。
- [x] 新 release 不可覆盖，共享环境文件权限保持 `0600`，迁移前备份非空。
- [x] 完整部署成功，数据库为 `0039_article_delete_platform`，全部服务健康。
- [x] 公网健康、缓存、安全头、SPA 和对象存储代理检查通过，Nginx 配置有效。
- [x] 登录后浏览器只读验收通过，发布成果页正常，不执行生产业务写入。
- [x] 全部验收通过后 `current` 精确指向新 release，并记录 commit、release、备份、迁移和验收结果。
