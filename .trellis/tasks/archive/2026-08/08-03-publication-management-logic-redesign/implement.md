...
> 2026-08-03 用户已批准 `prd.md`、`design.md` 与本计划；代码实现、环境重置、提交、预发布部署和业务验收均已完成。
...
- [x] 本地开发环境完成重建，迁移至 `0034_publication_redesign`，恢复后的真实 E2E 共享准备与主闭环 2 项通过，用时 48.8 秒。
- [x] 预发布备份 `/root/partsignal/backups/partsignal-20260803T095213Z.sql.gz` 已在隔离 PostgreSQL 中恢复验证，旧 5 条发布记录完整可读；随后重建业务库并部署 `mvp-20260803-174950-deb4286ea0c8`。
- [x] 预发布 UAT `c9177621` 完成失败核验后复核成功与带原因关闭两条分支；工作 `5d83fc70-6169-447e-a581-44ea84f17d9e` 形成只读成果，工作 `e4514338-283a-4ebd-8348-5c1e83f2451d` 关闭并取消来源任务，最终只读 Playwright 验收 1 项通过，用时 10.0 秒。

## 8. 环境重置与上线验证

- [x] 先在隔离测试数据库证明：非空旧数据使 `0034` 明确失败，空库可完整升级到 head。
- [x] 本地开发环境核对 Compose 项目和卷名后重建数据库卷，运行迁移、`seed-demo` 和真实 E2E 数据准备。
- [x] 预发布环境停止写入、记录准确数据库、运行 `deploy/scripts/backup.sh` 并验证备份存在后，再按已批准窗口重建数据库。
- [x] 预发布从空库迁移到 head、seed 账号、部署应用，执行登录、发布闭环和发布管理业务 UAT；GEO 候选与问题回流由同版本隔离真实 E2E 覆盖。
...
- [x] 新空库、开发重置库和预发布重置库描述同一 head schema。
...
- [x] 提交前已给出精确 commit plan 并取得用户确认；工作提交后按用户后续明确指示推送 `main`。
