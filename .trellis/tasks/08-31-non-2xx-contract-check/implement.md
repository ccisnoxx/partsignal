...
当前只完成规划。禁止在人工批准前运行 `task.py start`、创建实现子任务、修改代码/OpenAPI/generated client 或执行生产调用。

本 Task 推荐作为 3E 集成父任务，不直接承载一个跨 19+ 文件、162 operation 的不可评审实现 diff。批准后按下列顺序创建/启动子任务；父任务只拥有来源要求、波次依赖、最终集成验收和零漂移关闭条件。
...
- [ ] 抽出 document-level pure comparator，保留现有 operation/request/security 检查。
- [ ] 实现 status、response、header/media/schema normalization 和 RFC 6901 diagnostics。
- [ ] 增加 synthetic mutation matrix 与 FastAPI 0.139.0 最小 app 测试；覆盖 header ref/碰撞/schema-content 互斥和保留 `oneOf` 重复分支。
- [ ] 增加调用同一纯 comparator 的 `--response-report` 只读诊断入口；全量漂移时非零退出，不支持 filter、suppress 或 allowlist。
- [ ] `check()` 暂不调用新 response comparator；不得添加 ignored status/path、漂移 baseline 或 permanent report-only success。
- [ ] reviewer 独立检查 comparator 盲点和测试是否只复制实现。
...
- [ ] 逐 operation 复核现有和缺失的 401/403/404/409/422/5xx，保存精确 final matrix。
- [ ] 建立 response-header authority matrix：全局 `X-Request-ID`、两个 CSV `Content-Disposition` 进入 parity；login/logout 多实例 `Set-Cookie` 明确由 HTTP sentinel 保证，不伪造单值 OpenAPI schema。
- [ ] 用实际 handler sentinel 冻结 ErrorEnvelope 的 `code/message/details/request_id` 均 required，并同步静态 schema。
- [ ] 删除 `testAIModel` 不会逃逸的 502/504；补 `completeFileUpload` 的 503 及其他已证明状态。
- [ ] 冻结 health nullable、GEO discriminator、generation union 与 CSV header/media 的正确公共语义。
- [ ] 单独 review 并冻结 `X-Request-ID` request Header、400 response 及所有 response 同名 Header 的 final matrix；本阶段不写入根合同，避免在 runtime owner 尚未同步时让现有 request-parameter gate 失败，静态/runtime 同步统一由 Phase X 完成。
- [ ] 重新生成 client，并更新因精确 response sets 改变而失效的静态合同测试。
- [ ] critical reviewer 检查公共合同没有批量猜测、行为变更或丢失既有响应。
...
- [ ] 建立唯一 `ErrorEnvelope` wire schema 与 response metadata helper。
- [ ] 同步 health、auth/users/audit、configuration/AI、files 的逐 operation status。
- [ ] 修正两个 CSV response 的 media/header runtime metadata。
- [ ] 验证显式 422 均指向项目 ErrorEnvelope，实际非法输入行为不变。
- [ ] reviewer 检查没有把共享 helper 变成全 operation 状态模板。
...
逐 operation 对照 Phase B final matrix；只改 decorator metadata，不改 service、权限、状态转换、事务或错误映射。
...
逐 operation 对照 Phase B final matrix；`discoverAIChannelModels`/`testAIModel`、ready/logo/file 5xx 等特殊 owner 必须保留行为差异，不套统一模板。
...
- [ ] 按 Phase B 已批准 matrix 同步冻结合同与 runtime OpenAPI：从 request-context 代码自身的共享常量/metadata owner 为全部 162 个 operation 声明 X-Request-ID request Header、400 ErrorEnvelope 与每个 response 的 X-Request-ID Header。
- [ ] 不读取 `contracts/openapi.yaml`，不修改 operation-specific status owner，不用 `default`/`4XX` 掩盖显式 400。
- [ ] HTTP sentinel 证明非法 Header 返回 400 ErrorEnvelope、正常和错误响应都回写 request id。
- [ ] login/logout sentinel 证明两个 `Set-Cookie` 实例及关键安全属性保持；不把它们伪装成单值 OpenAPI Header。
- [ ] 在三个 domain wave 已完成的前提下，全量 `--response-report` 必须零漂移；Phase F 只负责切换默认门禁，不再承担 metadata 修复。
...
- [ ] `check()` 调用完整 response comparator，删除 `successful_response()` 和首个 2xx shortcut。
- [ ] 删除任何仅供过渡的 report-only 入口；保留纯 comparator 测试 seam。
- [ ] 确认完整 162 operation status/media/header/schema 零漂移。
- [ ] `make contract-check` 先执行 backend gate，再执行同一 frontend generated check。
- [ ] 检查 `.github/workflows/ci.yml` 仍通过 `make contract-check` 调用同一门禁，不复制命令。
- [ ] trellis-check 全范围审查；公共合同/跨模块变更使用 critical reviewer。
...
不默认运行 backend PostgreSQL integration、完整 build、Playwright 或 `make verify`：计划只改 checker、OpenAPI/generated 与 FastAPI metadata，不改数据库、权限、状态转换或业务命令。若 implementation diff 实际触及共享错误行为、公共响应模型序列化或发布门禁，则把相应 integration/build 升级为 Required。
...
- [ ] 人工确认当前 Task 作为集成父任务、批准后先创建 Phase A 子任务而非启动父任务。
- [ ] 人工确认 headers 进入机器合同，links 采用 unsupported fail-closed，annotation 字段不比较。
- [ ] 人工确认 `testAIModel` 以现有 200 失败投影为权威，不在 3E 改业务行为。
- [ ] 人工确认 `X-Request-ID` request/response Header 与 400 进入跨切面同步，`Set-Cookie` 留在多实例 HTTP sentinel。
- [ ] 人工确认 `ErrorDetail.details` 按实际 handler 改为 required。
- [ ] 人工确认 public contract 变更与 generated client 影响范围。
- [ ] 批准前 `task.py start` 未运行，`v2-live-readonly-acceptance` 仍为当前 in_progress Task。
