...
test('批准事实到人工发布、GEO 观测及删除与归档生命周期保持完整追溯', async ({ page, browser }, testInfo) => {
  ...
  const testedTimeoutModel = await command(page, `/api/v1/ai-models/${timeoutModel.id as string}/test`, csrf, { expected_revision: timeoutModel.revision });
  ...
  const invalidatedTimeoutModels = await body<{ items: Array<{ id: string; revision: number }> }>(await page.request.get(`/api/v1/ai-channels/${secondChannel.id as string}/models`));
  const invalidatedTimeoutModel = invalidatedTimeoutModels.items.find((item) => item.id === timeoutModel.id);
