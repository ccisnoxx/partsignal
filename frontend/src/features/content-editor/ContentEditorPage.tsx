...
export function ContentEditorPage() {
  ...
  return (
    <div className="page-stack">
      <Link to={`/tasks/${current.task_id}`}><ArrowLeftOutlined /> 返回内容任务</Link>
      <header className="page-heading">
        <div>
          <Typography.Text className="eyebrow">CONTENT VERSION / V{current.version}</Typography.Text>
          <Typography.Title>{current.title}</Typography.Title>
          <Typography.Paragraph>
            {current.source_type === 'AI' ? 'AI 草稿' : '人工修订'} · 哈希 {current.content_hash.slice(0, 12)}
          </Typography.Paragraph>
        </div>
        <Space wrap>
      ...
      {(revise.error || command.error) && (
        <Alert type="error" showIcon message={errorMessage(revise.error ?? command.error)} />
      )}
      <Row gutter={[16, 16]}>
        <Col xs={24} xl={12}>
          <Card title="当前 Markdown 正文">
            <Input.TextArea rows={18} readOnly value={current.body_markdown} />
          </Card>
        </Col>
        <Col xs={24} xl={12}>
          <Card title="安全预览">
      ...
      <Card title="质量问题">
        {current.quality_issues.length === 0 ? (
          <Typography.Text type="secondary">当前版本没有质量问题。</Typography.Text>
        ) : (
          <List
            dataSource={current.quality_issues}
            renderItem={(issue) => (
              <List.Item>
                <Space>
                  <Tag color={issue.severity === 'BLOCKING' ? 'red' : 'gold'}>
      ...
      <Card title={`任务锁定事实 V${review.fact_version.version}`}>
        <Descriptions
          column={1}
          items={[
