...
export function PlatformTypesPage() {
  ...
  const remove = useMutation({ mutationFn: async (row: PlatformType) => ensureSuccess(await api.DELETE('/api/v1/platform-types/{platform_type_id}', { params: { path: { platform_type_id: row.id }, query: { expected_revision: row.revision }, header: csrfHeader() } })), onSuccess: async () => { message.success('平台类型已删除'); await refresh(); } });
