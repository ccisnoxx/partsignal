...
export function ChangePasswordPage() {
  ...
  const change = useMutation({
    mutationFn: async (body: Schema<'ChangePasswordRequest'>) => ensureSuccess(await api.POST('/api/v1/auth/change-password', { params: { header: csrfHeader() }, body })),
    onSuccess: async () => { await auth.refresh(); },
  });
  useEffect(() => {
    if (change.isSuccess && auth.user?.must_change_password === false) navigate('/', { replace: true });
  }, [auth.user?.must_change_password, change.isSuccess, navigate]);
