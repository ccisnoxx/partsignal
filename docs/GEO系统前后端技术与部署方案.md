...
直接使用 HTTPX 或模型官方 SDK实现 `ContentGenerator` 适配器。当前没有必要增加 LangChain、Agent 框架或图编排系统。
...
- 网络超时和明确的服务端暂时错误可以有限重试。
- 模型返回不符合结构时记录失败，不通过无限重试掩盖提示或契约问题。
- 同一幂等键只能生成一个有效任务。
- 重试生成的草稿必须能够关联原任务和尝试次数。
- 错误日志不保存模型密钥、未公开资料全文或个人敏感信息。
