...
`COMPLETED` 记录任务已完成首次核验闭环，是不可回退的历史终态。非终态发布工作存在时不能直接取消任务；关闭发布工作会在同一事务把来源任务置为 `CANCELLED`。生成和发布进度通过作业、内容版本与发布工作推导，不维护重复状态。

`CANCELLED` 任务可由普通用户连同其生成作业、内容审核记录和 `DRAFT | PENDING_REVIEW | CHANGES_REQUESTED` 内容版本物理删除。存在 `APPROVED | SUPERSEDED` 内容、任一发布工作或 `source_published_content_issue_id` 时服务端返回真实阻断引用；前端只消费 `available_actions`，不自行推导删除资格。
...
发布成果形成后出现页面不可用、公开内容变化或其他问题时，创建唯一 `OPEN PublishedContentIssue`。问题详情保留原成果、原任务、问题类型、说明和完整审计，不回退已完成内容任务，也不改写或删除成果。
...
管理员可以清理没有历史引用的当前开发数据；内容任务是明确例外，普通用户可按服务端 `available_actions` 清理没有受保护历史的已取消任务。服务端锁定目标并统计阻断引用，存在受保护关系时返回对象类型与数量；前端只负责中文展示，不复制引用判断。
...
| 平台账号 | 发布工作 | 只删除未用于发布的内部运营账号标识 |
| 平台类型 | 具体平台 | 不自动删除具体平台 |
| 已取消内容任务 | `APPROVED / SUPERSEDED` 内容、发布工作、发布后问题修复来源 | 删除该任务的生成作业、审核记录和未批准内容版本；不清理下游历史 |
...
- 已取消任务可连同生成作业、审核记录和未批准内容版本删除；已批准/曾批准内容、发布工作或发布后问题修复来源返回结构化引用冲突且保持不变。
