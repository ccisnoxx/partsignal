...
### 2.2 2026-07-16 平台配置治理

当前实现采用动态平台分类和具体平台单次生成，不建设批量分类生成或二次平台适配流程：

- 产品事实覆盖产品参数、特性、认证、应用范围、测试结果和证据；替代关系只是可选事实。
- `PlatformType` 是管理员维护的业务分类；`PlatformProfile` 表示具体平台，并拥有零或一份当前 Markdown Prompt。
- `PlatformProfileVersion` 保存具体平台不可变规则。工程师创建内容任务时直接选择一个 `ACTIVE` 规则版本；所属平台还必须存在当前 Prompt。
- 删除未引用的 `ACTIVE` 规则后，平台保留并进入“无有效规则”状态。管理员仍可配置该平台，工程师在新版本激活且 Prompt 存在前不能选择它。
- 每次生成作业冻结具体平台身份、平台规则、最终 system/user message、事实、模型和参数；当前配置后续变化不影响历史作业。
...
- `PlatformProfile.is_active` 是平台自身启停的唯一状态，既有平台迁移为启用。停用与配置完整性相互独立；停用平台仍可查看、编辑、维护规则和 Prompt 及重新启用。
- 停用不联动既有账号，也不改写规则、Prompt、内容任务、发布记录、GEO 观测或审计历史；服务端在新建普通/修复任务、平台账号和发布记录前锁定平台并明确拒绝停用状态。
- 平台列表搜索、类型/状态/完整性筛选、分页、详情、实时汇总和 CSV 导出使用同一 PostgreSQL 权威集合。五张统计卡只显示当前实时计数，历史趋势没有快照来源时统一显示“暂无历史基线”。
- 平台引用数按引用该平台任一规则版本的唯一内容任务统计；最近 30 天使用同一 UTC `as_of` 的半开区间 `[as_of - 30 days, as_of)`，查看引用分析进入按平台筛选的内容任务列表。

### 2.5 2026-07-22 平台规则版本闭环

- 平台规则页采用平台、版本、规则详情和版本信息四区工作台；平台搜索/状态、选中平台和选中版本写入 URL，深链无效时局部报错，不静默改选其他对象。
- 管理投影只展示现有 `PlatformRules` 字段，并从规则版本、任务引用和审计实时批量计算引用数、创建/激活/最后变更时间及可用动作；不新增规则字段、汇总表或第二状态源。
- 激活 `DRAFT` 替换既有 `ACTIVE` 时，旧版本才自动退役，并在同一事务记录旧版本替代退役与新版本激活两条审计；直接退役仅适用于草稿，二者都要求非空操作说明和最新 `revision`。
- 影响摘要按直接引用任务互斥统计：优先归入“当前已发布”，其次“审核中”，其余“未发布”；查看引用进入按精确规则版本过滤的任务列表，若同时存在平台筛选则取交集。
...
- 平台列表从同一服务端集合读取真实 Logo、类型、ACTIVE 规则、配置状态和当前 Prompt 更新时间。`prompt_updated_at` 只来自当前 Prompt 行；平台更新或审计时间不冒充 Prompt 时间。
...
- 侧栏把发布账号、历史目标问题、用户管理和审计日志归入“业务设置”；用户管理与审计日志只对管理员显示，配置中心不再重复这两个入口。
- 用户工作台的搜索、账号类型、状态、稳定排序和分页全部由服务端 PostgreSQL 查询负责；五张统计卡使用未筛选实时汇总，没有历史快照时明确显示“暂无历史基线”。
- 管理员新增账号时提交一次性临时密码；服务端只保存哈希，新账号默认启用且 `must_change_password=true`。重置临时密码或停用账号会撤销目标用户全部活动会话。
...
- 日常用户生命周期仍只有启用、停用、改名和账号类型调整，不提供用户删除。相同用户 UUID 持续承载历史业务归属，停用或重新启用不改写业务记录与既有审计。
...
13. 平台类型由管理员维护；具体平台归属于一个类型，生成时必须锁定明确的 `PlatformProfileVersion`。
14. 只有使用明确具体平台规则生成并批准的内容才能创建发布记录。
...
| 系统管理员 | 系统维护人员 | 拥有普通用户的业务能力，并额外管理平台类型、具体平台、平台 Prompt 与规则版本、用户、AI 渠道、Header、模型和系统安全配置 |
...
系统只保存 `ADMIN` / `ENGINEER` 账号类型和启停状态，不建设角色表、权限配置界面或自定义权限引擎。所有写操作都必须记录实际操作者，并在服务端同时校验账号状态、管理员能力和业务状态转换。普通用户可以在任务中只读查看所选平台 Prompt 和规则，但不能修改配置，也不能管理渠道和模型。
...
初始化命令必须幂等，重复部署不得覆盖任何既有密码。所有登录用户都能从顶部账号区域进入自助改密，修改时必须验证当前密码并撤销其他会话；管理员只能通过该流程修改自己的密码。管理员可以新增用户、编辑显示名称与账号类型、设置其他用户的临时密码并单个或批量启停账号，长期用户不提供物理删除。管理员调整或停用自己时也必须满足“提交后仍至少有一个有效管理员”；自我停用成功后当前会话立即失效。
...
flowchart TD
    ...
    B --> C{事实审核}
    C -->|退回| A
    C -->|批准| D[选择具体平台和 ACTIVE 规则创建任务]
    D --> E[读取平台当前 Prompt 并生成草稿]
    E --> F{内容审核}
    F -->|退回| E
    F -->|批准| J[待人工发布]
    J --> K[用户在平台发布]
...
1. 普通用户创建内容任务，选择产品、已批准事实版本和具体平台的当前 `ACTIVE PlatformProfileVersion`，再填写受众、内容角度、转化目标、格式、长度和官网权威页。
2. 用户维护产品描述、参数、特性、应用边界和可选产品对比等专业 Markdown 输入，任务绑定的已批准事实继续作为只读权威来源。
3. 系统从规则版本解析具体平台，并要求该版本仍为 `ACTIVE` 且平台存在当前 `PlatformPrompt`；任一条件缺失都明确拒绝创建任务或作业。
4. 用户选择模型后创建一个生成作业。作业冻结渠道非敏感信息、模型参数、具体平台身份、平台规则、最终 system/user message、事实和任务要求；缺失 token 用量保持为空。
5. 用户编辑并审核 Markdown 草稿；批准后才进入待发布列表。重试创建新作业并复用原不可变快照。

### 7.3 人工发布流程

1. 已批准且绑定明确具体平台规则的内容进入“待人工发布”列表。
2. 用户在按需 Drawer 中读取不可变发布包，选择锁定平台下的匹配账号和目标栏目 URL；没有匹配账号时从工作台进入业务设置。系统先建立 `PENDING_MANUAL_PUBLISH PublicationRecord` 并固定内容版本。
...
| `QueryTopic` | 历史目标问题 | 保存旧内容任务和旧模型观测使用的问题、意图分类和关键词变体 |
...
| `ContentTask` | 内容任务 | 定义为哪个产品和具体平台生产什么内容；历史任务可保留目标问题关联 |
...
erDiagram
    ...
    PLATFORM_PROFILE_VERSION ||--o{ CONTENT_TASK : selected_by
...
`ContentTask` 绑定一个具体 `platform_profile_version_id`，后续可以继续修改任务级 `user_prompt_markdown`。每次生成单独选择一个已启用模型；`GenerationJob.input_snapshot` 才是某次实际模型调用上下文的权威记录，不能用任务、Prompt、平台规则或模型配置后来更新的当前值解释历史内容。
...
`PlatformProfile` 表示知乎或电子发烧友等具体平台，保存名称、唯一 `slug`、允许域名、所属类型和独立 `is_active` 状态。创建平台只创建已启用身份，不隐式创建规则版本，因此新平台可以处于“无有效规则”；启用状态不表示配置完整。每个平台最多拥有一条当前 `PlatformPrompt`，正文是普通 Markdown，不支持变量、循环、条件或 Prompt 编排。管理员可以覆盖或删除当前 Prompt；历史含义由生成作业的最终 system message 冻结。

`PlatformProfileVersion` 只保存目标读者、标题/正文长度、语气、外链/表格/联系方式开关、禁用表达和栏目地址等现有平台规则。规则版本使用 `DRAFT → ACTIVE → RETIRED` 生命周期；`DRAFT` 按 `revision` 并发控制编辑，`ACTIVE` 或 `RETIRED` 后不可编辑。每个平台同时最多一个 `ACTIVE` 版本，平台当前规则始终由该唯一版本推导，不保存第二个当前规则字段。激活草稿替换当前版本时旧版本才自动退役，并在同一事务记录替代退役与激活审计。
...
旧模型观测继续保留原目标问题、模型、联网标识、回答摘要、引用和准确性等历史字段，但不再用于创建新的人工观测。人工观测不保存模型回答摘要。人工更正通过追加同产品、同问题主题、同搜索平台和同搜索词的新记录完成，不覆盖原记录；补采前主题为空的历史记录首次更正时必须补充真实主题。默认列表和指标只读取更正链尾，显式选择“包含历史更正记录”时才读取完整历史。
...
- AI 输出固定为 `DRAFT`。
- 只有绑定明确具体平台规则的 `APPROVED` 内容才能创建发布记录。
...
flowchart LR
    ...
    A --> C[加载所选 ACTIVE PlatformProfileVersion]
    C --> D[加载所属 PlatformProfile 当前 Prompt]
...
云端模型只接收允许的文本事实值，不接收证据文件、附件、私有源文档、对象存储地址或访问凭据。新任务要求包含产品、目标受众、内容角度、期望长度、转化目标和权威页地址；历史任务创建新作业时还会保留其真实目标问题。

服务端使用明确白名单组装 user message，不允许模型读取未批准事实草稿或任意历史字段。生成请求只提交 `ai_model_id`；平台、Prompt、事实和任务草稿全部由服务端权威解析。

### 10.3 平台配置

`PlatformProfile` 表示具体平台官网，保存平台名称、Slug、所属动态业务类型、官网和允许域名；`PlatformProfileVersion` 保存业务约束，不保存页面自动化选择器：
...
具体平台必须归属于一个 `PlatformType`。平台规则使用不可变版本约束标题、正文长度、外链、表格、联系方式、语气和禁用表达；具体平台拥有当前唯一的 Prompt Markdown。
...
生成内容时，服务端读取所选 `PlatformProfileVersion` 的完整规则和所属平台当前 Prompt。规则版本必须为 `ACTIVE` 且 Prompt 必须存在；否则明确阻止生成，不自动套用其他版本、其他平台或类型级 Prompt。具体平台身份、规则和最终 system message 均写入新作业快照。
...
创建修复任务与解决异常是两个独立操作。修复任务固定继承原产品和平台；历史任务继续继承其真实目标问题，新产品驱动任务保持目标问题为空。普通用户必须从同产品当前 `APPROVED` 事实版本和原平台当前 `ACTIVE` 规则版本中重新选择，并查看与原版本的字段化差异。目标受众、内容角度、转化目标、格式、长度和 `canonical_url` 从原任务预填但可以编辑。创建修复任务只建立来源关联，不自动关闭异常待办。
...
| `content_tasks` | `id`, `product_id`, `fact_version_id`, 可空历史 `query_topic_id`, `user_prompt_markdown`, `status` |
...
- 内容任务必须引用一个 `ACTIVE` 平台规则版本；生成作业必须冻结该规则所属具体平台和当前 Prompt。
- 发布记录只能绑定已批准内容，且平台账号必须与任务锁定规则所属具体平台一致。
...
- 管理员新增账号、编辑显示名称和账号类型、设置临时密码，以及按 revision 单个或批量启停用户；服务端最终保护最后一个有效管理员并撤销被停用用户会话。
- 管理员按当前筛选导出唯一 CSV 格式；导出和用户变更写入脱敏审计。
- 用户不存在日常删除接口；停用、重新启用和资料调整保留稳定 UUID 及全部历史引用。
...
- 左侧：产品、所选具体平台、只读 Prompt、规则版本、模型选择、工程师输入和内容任务。
- 中间：标题、摘要和 Markdown 正文编辑。
- 右侧：产品事实、证据、可选替代边界、平台规则、生成输入快照和质量警告。

应支持查看 AI 原始版本与当前版本差异，并明确显示当前绑定的事实版本、具体平台 Prompt、平台规则版本、完整 system/user message 快照、渠道、模型和参数。普通用户只能查看 Prompt 和平台规则，不能修改。

配置中心仅对管理员显示并由服务端限制访问；普通用户通过内容任务页面查看所选具体平台 Prompt，不进入配置页面。

Prompt 管理使用高密度三栏桌面工作台：左侧为可搜索、按类型筛选和分页的平台集合，中间为带行号、字数/行数、保存状态和 revision 元数据的唯一 Markdown 编辑区，右侧为真实输出预览与固定安全边界。全局自然化标签隐藏平台列表但复用编辑与预览区域；窄屏按平台列表、编辑器、预览/安全说明顺序堆叠，页面本身不产生横向滚动。
...
管理员可以新增、编辑、重置临时密码、单个或批量启停以及导出当前筛选。所有状态与权限以服务端响应为准，前端不从分页结果推算汇总、最后管理员或会话影响。加载、空、部分失败、revision 冲突和最后管理员保护均显式展示；页面不提供删除、虚构趋势、跨页隐式选择或无后端能力的快捷入口。
...
- 发布登记、URL 变更、拒绝和下线。
- GEO 观测创建和人工准确性判断。
- 平台 Prompt 覆盖、具体平台及规则版本创建/启停，AI 渠道、API Key 重新配置、Header、模型发现/测试/变更及启停。
- 用户账号、管理员标识和关键系统配置变更。
- 用户列表导出；摘要只保存非敏感筛选和值域及导出行数，不保存 CSV 正文。

审计日志保存变更摘要，不保存 `system_prompt` 正文、模型 API 密钥、平台 Cookie 或其他敏感值。

`audit_logs` 是业务审计唯一来源，并由 PostgreSQL 触发器保持追加后不可修改或删除。成功事件与业务写入同事务提交；当前仅对事实版本状态转换、内容提交与审核、发布登记、GEO 观测、平台与规则、平台 Prompt、AI 渠道与模型、用户状态与管理员标识、用户导出九类关键命令，在业务事务回滚后以独立事务记录 `FAILED` 或 `DENIED`。请求解析、身份认证、会话与 CSRF 失败不属于业务命令审计。
...
| 所选具体平台缺少当前 Prompt | 阻止创建内容任务或生成作业，不回退到类型级 Prompt |
| 具体平台缺少 `ACTIVE` 规则版本 | 工程师不可选择；管理员仍可创建并激活新规则版本 |
...
- 平台类型由管理员动态维护；具体平台规则必须归属一个真实具体平台。
- 平台规则管理投影的引用、审计元数据和可用动作必须来自批量权威查询，不允许前端逐行补算。
- 影响摘要必须按“当前已发布 → 审核中 → 未发布”优先级互斥分桶，三桶之和等于直接引用任务总数。
- 激活替代必须原子记录旧版本替代退役和新版本激活，任一失败时状态与审计整体回滚。
...
- 事实版本创建、审核和冻结。
- 动态平台类型、具体平台 Prompt 覆盖、规则版本生命周期、替代激活双审计、互斥影响摘要、普通用户只读查看和管理员权限校验。
...
```text
管理员配置平台类型、具体平台 Prompt、平台规则、渠道 Header 和模型
→ 模型严格测试通过并启用
→ 录入产品事实
→ 批准事实版本
→ 创建内容任务并填写工程师输入
...
2. 证据上传和事实版本审核。
3. 历史目标问题、动态平台类型、具体平台 Prompt，以及独立的平台规则草稿与生命周期管理。
...
- 实现问题库、动态平台类型、具体平台 Prompt、具体平台规则版本和单平台内容任务。
...
5. 每个内容任务锁定一个具体平台的 `ACTIVE` 规则版本；该平台存在当前 Prompt 时才能生成草稿。
6. 生成内容能够追溯到事实版本、具体平台 Prompt、平台规则版本、完整 system/user message 快照、渠道、模型和生成参数。
...
15. 关键事实、Prompt、平台规则、生成、审核和发布操作均有审计记录。
16. 完整主流程有自动化测试或明确的可重复验收脚本。
17. API Key 和敏感 Header 使用数据库密文且永不回显，生产环境拒绝非公网 HTTPS、重定向和保留 Header 覆盖。
18. 空数据库初始化得到一个管理员和一个强制首次改密的普通用户；管理员可以查询、分页、新增、编辑、单个/批量启停、重置其他用户密码和导出安全 CSV，并能修改自己的密码。最后有效管理员不可被移除，用户历史归属不会因启停或资料调整被改写。
...
方案通过不可变事实版本、具体平台 Prompt 与规则快照、内容版本、标准发布记录和追加式 GEO 观测建立清晰的数据所有者。该结构既能支持当前人工发布，也为未来按具体平台逐步增加自动发布保留了稳定边界，同时避免在业务价值尚未验证前引入跨平台 RPA 的复杂度。
