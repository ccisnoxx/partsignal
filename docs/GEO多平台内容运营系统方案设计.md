...
- 管理员新增账号时提交一次性临时密码；服务端只保存哈希，新账号默认启用且 `must_change_password=true`。重置临时密码最少 8 位，重置或停用账号会撤销目标用户全部活动会话；新建账号临时密码和用户自助改密继续采用既有 12 位边界。
...
- 重置临时密码接受 8 位边界并拒绝 7 位；新建账号和用户自助改密保持既有 12 位边界。
