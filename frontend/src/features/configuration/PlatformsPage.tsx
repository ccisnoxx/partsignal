...
export function PlatformsPage() {
  ...
  const updateProfile = useMutation({
    ...
    onSuccess: async () => { setEditProfile(null); await queryClient.invalidateQueries({ queryKey: queryKeys.platformProfiles.all }); },
  });
  ...
  const removeProfile = useMutation({
    ...
    onSuccess: async () => queryClient.invalidateQueries({ queryKey: queryKeys.platformProfiles.all }),
  });
  ...
  return <div className="page-stack">
    ...
    <Card className="collection-panel">{platforms.isLoading || versions.isLoading ? <QueryLoading label="正在加载平台" /> : queryError ? <QueryFailure error={queryError} onRetry={() => { void platforms.refetch(); void versions.refetch(); }} /> : platformItems.length === 0 ? <NoData description="暂无具体平台" /> : <TableRegion label="平台列表"><Table<PlatformProfile> rowKey="id" dataSource={platformItems} scroll={{ x: 980 }} columns={[
      ...
      { title: '操作', render: (_, profile) => <Space><Button size="small" onClick={() => setEditProfile(profile)}>编辑归类</Button><Popconfirm title={`物理删除平台“${profile.name}”？`} description="必须先清理全部规则版本和平台账号；当前 Prompt 会一并删除，历史作业不受影响。" okText="删除" cancelText="取消" onConfirm={() => removeProfile.mutate(profile)}><Button size="small" danger icon={<DeleteOutlined />}>删除平台</Button></Popconfirm></Space> },
    ]} /></TableRegion>}</Card>
