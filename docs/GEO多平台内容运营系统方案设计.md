...
- 已取消内容任务可连同生成作业、内容审核记录和 `DRAFT | PENDING_REVIEW | CHANGES_REQUESTED` 内容版本物理删除；存在 `APPROVED | SUPERSEDED` 内容、任一发布记录或发布修复来源时服务端返回结构化冲突，不清理任何下游历史。
...
`CANCELLED` 任务可由普通用户连同其生成作业、内容审核记录和 `DRAFT | PENDING_REVIEW | CHANGES_REQUESTED` 内容版本物理删除。存在 `APPROVED | SUPERSEDED` 内容、任一发布记录或发布修复来源时服务端返回真实阻断引用；前端只消费 `available_actions`，不自行推导删除资格。
...
| 平台类型 | 具体平台 | 不自动删除具体平台 |
| 已取消内容任务 | `APPROVED / SUPERSEDED` 内容、发布记录、发布修复来源 | 删除该任务的生成作业、审核记录和未批准内容版本；不清理下游历史 |
...
- 普通用户可删除已取消任务及其生成作业、审核记录和未批准内容版本；其他状态以及存在已批准/曾批准内容、发布记录或发布修复来源的任务由服务端拒绝。
...
- 表现最佳：当前周期至少 3 次完整观测，依次按准确率、提及率、发现率、观测次数降序，最后按 `publication_record_id` 升序稳定排序；不使用加权综合分。
...
- 已取消任务可连同生成作业、审核记录和未批准内容版本删除；已批准/曾批准内容、发布记录或发布修复来源返回结构化引用冲突且保持不变。
