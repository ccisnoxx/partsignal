...
export function PlatformTypesPage() {
  ...
  const prompt = useQuery({ queryKey: ['platform-prompt', selected?.id], queryFn: async () => unwrap(await api.GET('/api/v1/platform-types/{platform_type_id}/prompt', { params: { path: { platform_type_id: selected?.id ?? '' } } })), enabled: !!selected, staleTime: QUERY_STALE_TIME.configuration, retry: false });
  const refresh = async () => queryClient.invalidateQueries({ queryKey: ['platform-types'] });
