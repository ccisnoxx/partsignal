...
- 产品、事实版本、平台类型、具体平台、发布账号、已取消任务和停用用户存在直接引用时，更多菜单显示“查看删除条件”，列出引用类型与数量并跳转到带精确筛选的引用页面。发布工作、已批准或曾批准内容、GEO 来源、审计等不可变历史只提供“查看历史”；界面不级联或强制清理，引用处理后由用户主动重新检查服务端投影。
...
- 用户物理删除只允许停用且没有业务历史引用的账号。管理列表预先返回业务历史引用总数，删除命令锁定账号后按同一口径复核；会话随外键级联清理，历史审计操作者由 `0027` 受约束置空，活动账号和业务引用冲突由服务端明确拒绝。
...
七类当前开发数据使用同一受约束删除口径：产品检查事实版本、内容任务和 GEO 观测；事实版本检查内容任务和内容版本；平台类型检查具体平台；具体平台检查内容任务和平台账号；平台账号检查发布工作；已取消任务检查已批准或曾批准内容、发布工作、发布后问题修复来源和 GEO 优化来源；停用用户汇总全部业务历史外键。列表与详情返回 required nullable `deletion`：没有管理上下文为 `null`，可删除为 `blockers=[]` 并包含 `DELETE`，被引用时返回类型与正整数数量且不包含 `DELETE`。删除命令锁定目标后按同一口径复核，冲突统一返回结构化 `409`。无引用事实版本可在任意状态删除，其从属事实审核记录在同一事务显式清理并保留安全审计摘要；产品不会自动删除事实版本，停用用户删除只清理会话并按 `0027` 保留审计历史。除 `0033` 明确允许清理的任务自有未批准历史外，其他删除不级联或改写业务历史。
...
- [ ] 七类受约束物理删除对象被引用时显示类型、数量和精确下钻；不可变历史只查看，处理可删除引用后可重新检查并恢复删除入口。
