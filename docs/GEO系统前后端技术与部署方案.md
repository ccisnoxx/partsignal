...
- 管理员重置其他用户的临时密码最少 8 位；新建用户与用户自助改密保持既有 12 位校验。
...
管理员删除当前开发数据时，服务先锁定目标并统计直接引用，冲突统一返回结构化 `409`：产品检查事实版本、内容任务和 GEO 观测；事实版本检查内容任务和内容版本；具体平台检查内容任务和平台账号；平台账号检查发布记录；平台类型检查具体平台；停用用户由既有业务外键阻断。无引用事实版本可在任意状态删除，其从属事实审核记录在同一事务显式清理并保留安全审计摘要；产品删除不会自动删除事实版本。已取消内容任务只有在没有生成作业和内容版本时可由普通用户删除；停用用户删除会清理会话并保留审计历史。其他删除不级联或改写业务历史。
...
- [ ] 已取消且无生成/内容历史的任务可删除，已有生产历史时返回真实引用冲突。
