...
- 首次成功核验原子形成只读发布成果并完成来源内容任务；显式关闭原子取消来源任务。成功成果不因后续页面问题回退或删除。
- GEO 新观测只消费没有开放问题且未退役的 `PublishedArticle`。打开问题、创建修复任务和解决问题分别是独立动作，任何一步都不代替其他步骤。
- 发布业务对象不提供物理删除。开发和预发布旧数据通过核对目标后的环境重置清理；迁移发现非空旧发布或 GEO 依赖时以 PostgreSQL `55000` 明确失败，不猜测映射。
...
- 发布成果：只展示首次核验成功形成的 `PublishedArticle`，详情只读，不提供修改或删除。
...
发布成果形成后出现页面不可用、公开内容变化或其他问题时，创建唯一 `OPEN PublishedContentIssue`。问题详情保留原成果、原任务、问题类型和说明，不回退已完成内容任务，也不通过日常发布命令改写或单项删除成果。
...
详情使用有选中对象才打开的右侧 Drawer；桌面保持可读宽度，移动端占满可用宽度。表格自身横向滚动，页面不得产生横向滚动。键盘可以操作页签、筛选、表格动作和 Drawer，关闭后焦点回到触发位置。只读发布成果不显示修改或删除；危险关闭必须明确说明来源任务将取消且工作不可恢复。
